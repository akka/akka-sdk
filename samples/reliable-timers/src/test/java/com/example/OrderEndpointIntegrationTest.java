...
public class OrderEndpointIntegrationTest extends TestKitSupport {
  ...
  private HttpResponse confirmOrder(String orderId) {
    return httpClient.POST("/orders/" + orderId + "/confirm").invoke().httpResponse();
  }

  private HttpResponse cancelOrder(String orderId) {
    return httpClient.POST("/orders/" + orderId + "/cancel").invoke().httpResponse();
  }
  ...
  private OrderStatus getOrderStatus(String orderId) {
    return componentClient.forKeyValueEntity(orderId).method(OrderEntity::status).invoke();
  }
}
