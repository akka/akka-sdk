...
class TransfersViewIntegrationTest extends TestKitSupport {
  ...
  public TestKit.Settings testKitSettings() {
    return TestKit.Settings.DEFAULT.withWorkflowIncomingMessages(TransferWorkflow.class);
  }
  ...
  public void shouldTestTransferViewWithPredefinedStateChanges() {
    ...
    var state1 = new TransferState(
      new TransferState.Transfer("from", "to", 100),
      TransferState.TransferStatus.STARTED
    );
    var state2 = new TransferState(
      new TransferState.Transfer("from", "to", 200),
