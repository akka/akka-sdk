...
public class WebSearchService {
  ...
  @FunctionTool(
    description = "Search the web for up-to-date information, news, and facts about a specific topic."
  )
  public String search(@Description("The search query or topic to look up") String query) {
    if (apiKey == null || apiKey.isEmpty() || cx == null || cx.isEmpty()) {
      logger.warn(
        "Google Search API credentials not configured. Returning simulated results."
      );
    ...
    try {
      ...
      String path = String.format(
        "/customsearch/v1?key=%s&cx=%s&q=%s",
        apiKey,
        cx,
        encodedQuery
      );
      ...
      if (response.httpResponse().status().isSuccess()) {
        ...
        GoogleResponse data = JsonSupport.getObjectMapper()
          .readValue(response.body().utf8String(), GoogleResponse.class);
        ...
        return data
          ...
          .limit(5)
          .map(
            item -> String.format("- %s: %s (%s)", item.title(), item.snippet(), item.link())
          )
          .collect(Collectors.joining("\n"));
      } else {
        logger.error(
          "Google Search API call failed with status: {}. Body: {}",
