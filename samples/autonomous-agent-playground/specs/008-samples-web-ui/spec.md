...
3. **Given** an event that represents a struggle or failure (e.g. iteration failed, task failed, task cancelled, dependency wait blocking progress, result rejected by a guard rule), **When** it is rendered, **Then** it is visually distinct from healthy events (color/icon/label) so it stands out at a glance.
...
- **SC-005**: Struggle and failure events are recognizable at a glance, distinct from healthy progress, in both light and dark themes.
