...
public class ShoppingCartEndpoint {
  ...
  public String create() {
    ...
    try {
      ...
      ShoppingCartMetrics.shoppingCartCreated(); // <1>
      return cartId;
    } catch (Exception e) {
  ...
  public ShoppingCartDTO verifiedAddItem(String cartId, LineItemDTO addLineItem) {
    if (addLineItem.name().equalsIgnoreCase("carrot")) {
      ...
      var addItemResult = componentClient
  ...
  public String unsafeValidation(String cartId, LineItemDTO addLineItem) {
    ...
    int totalCount = cart.items().stream().mapToInt(ShoppingCart.LineItem::quantity).sum();

    if (totalCount < 10) {
      throw HttpException.badRequest("Max 10 items in a cart");
    } else {
      var cartAfterAdd = componentClient
        .forKeyValueEntity(cartId)
        .method(ShoppingCartEntity::addItem)
        .invoke(addLineItem.toDomain());
