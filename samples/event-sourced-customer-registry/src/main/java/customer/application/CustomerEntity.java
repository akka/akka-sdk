...
public class CustomerEntity extends EventSourcedEntity<Customer, CustomerEvent> {
  ...
  public Effect<Done> changeName(String newName) {
    if (currentState() == null) {
      ...
      return effects().persist(new NameChanged(newName)).thenReply(__ -> done());
    }
  }
  ...
  public Effect<Done> changeAddress(Address newAddress) {
    if (currentState() == null) {
      ...
      return effects().persist(new AddressChanged(newAddress)).thenReply(__ -> done());
    }
  }
  ...
  public Customer applyEvent(CustomerEvent event) {
    return switch (event) {
      case CustomerCreated created -> new Customer(
        created.email(),
        created.name(),
        created.address()
      );
      case NameChanged nameChanged -> currentState().withName(nameChanged.newName());
      case AddressChanged addressChanged -> currentState()
        .withAddress(addressChanged.address());
    };
  }
