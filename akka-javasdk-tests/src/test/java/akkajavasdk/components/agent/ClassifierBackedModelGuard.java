...
/**
 * An AgentResponseGuardrail that delegates the decision to a configured classifier, resolved via
 * {@link GuardrailContext#classifierClient()}. Demonstrates the "used inline from a guardrail"
 * scenario.
 *
...
public class ClassifierBackedModelGuard implements AgentResponseGuardrail {
  ...
  public CompletionStage<Decision> decideAsync(CallContext ctx) {
    return classifierClient
        .classifyAsync(classifierName, ctx.reply().text())
        .thenApply(
            classification ->
                classification
                    .label()
