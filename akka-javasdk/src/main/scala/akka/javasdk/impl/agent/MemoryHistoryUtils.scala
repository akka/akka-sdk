 ...
 * INTERNAL API
 *
 * Shared filtering and trimming helpers for session memory history. Used by both
 * `SessionMemoryEntity#getHistory` (when reading the current entity state) and the agent runtime
 * (when falling back to a chunked journal read), so the two paths produce identical context for
 * the model.
 *
 * The public API uses Java collection types so the entity (which is implemented in Java) can call
 * it without conversion noise; Scala callers convert at the call site.
 */
@InternalApi
object MemoryHistoryUtils {
  ...
  /**
   * Resolve the role of an agent component, treating null/blank roles as "no role" so a role-based
   * filter does not accidentally match every agent.
   */
  ...
  /**
   * Keep only the last `n` messages. When `lastN` is empty or the list already has at most `n`
   * elements, the input list is returned untouched.
   */
  def trimToLastN(
      messages: util.List[SessionMessage],
      lastN: Optional[Integer]): util.List[SessionMessage] = {
