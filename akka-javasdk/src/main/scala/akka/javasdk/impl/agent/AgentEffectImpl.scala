...
import akka.runtime.sdk.spi.SpiAgent.MultiModalResponse
...
private[javasdk] final class BaseAgentEffectBuilder[Reply]
  ...
  override def responseAsMultiModal(): MappingResponseBuilder[MultiModalResponse] = {
    updateRequestModel(_.copy(responseType = classOf[MultiModalResponse], includeJsonSchema = false))
    new MappingResponseEffectBuilder(_primaryEffect.asInstanceOf[RequestModel])
  }
