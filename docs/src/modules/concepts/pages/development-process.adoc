...
* **Consumers and Views subscribing to external data** - When a Consumer or View subscribes to data from outside the service—either from a broker (e.g., Kafka) or via xref:sdk:service-to-service.adoc#_streaming_use_case[service-to-service streaming]—sharing allows multiple services to process the same external data stream or maintain the same projection.
...
* **Endpoints** - Sharing an endpoint means sharing all the components it depends on. If any dependency is a stateful component (Entity, Workflow), each service creates its own isolated data store—giving a misleading sense of shared data. Consider whether services truly need identical routes, or if one service should own the endpoint and others call it via xref:sdk:service-to-service.adoc[service-to-service calls].
