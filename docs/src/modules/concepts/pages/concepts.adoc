...
|One component can invoke another using a direct call. These invocations are non-blocking and asynchronous. The invoking component continues its work without waiting for a reply. This helps maintain responsiveness in the system.

A common example is a Workflow that performs a long-running process and invokes an Agent to handle a specific task. The Workflow can move on while the Agent does its work in parallel.
...
For example, when an Entity updates its state, it can emit an event. A View can subscribe to that event to stay in sync. Events can also come from external sources, such as APIs or streaming services.
|===

Akka encourages building systems where components are loosely coupled. Communication between them runs on virtual threads, which are lightweight and isolated. This allows each interaction to happen independently, helping to avoid contention and ensuring that system resources are used efficiently.

This model contributes to the scalability of Akka systems. It is suitable for handling high volumes of traffic, including systems that reach 10 million transactions per second.

The examples below illustrate typical interoperability patterns between components.
