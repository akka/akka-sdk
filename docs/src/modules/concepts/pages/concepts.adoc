...
|Tools
|Any public method made available to Model Context Protocol (MCP) clients.
...
You implement Endpoints in Akka using either xref:java:http-endpoints.adoc[HTTP], xref:java:grpc-endpoints.adoc[gRPC] or xref:java:mcp-endpoints.adoc[MCP] protocols.
...
You implement guardians in Akka by...
ifdef::todo[TODO: get some thoughts from Patrik on this]
|Adaptation
...
You implement a registry in Akka by... 
ifdef::todo[TODO: We need to talk to Patrik about this...]
|===
