...
For example, a Workflow component handling a long-running business transaction can invoke an Agent component to perform a task, and the Workflow will continue processing without waiting for the Agent to complete its work.
...
Due to this structure, even though the code you create which connects different components together, all communications between these components are decoupled from one another with non-blocking, asynchronous communication. This enables instances of your components to run anywhere with the most efficient use of compute, memory, and I/O while the Akka runtime takes responsibility for routing requests to the location that needs to consume them. This basic behavior is one reason why Akka services, whether stateful or stateless, can scale to 10M TPS.
