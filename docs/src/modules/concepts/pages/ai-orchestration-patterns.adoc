...
:page-aliases: concepts:ms-agent-patterns.adoc
...
== Route communication through a supervisor
...
Instead, route all coordination through a supervisor that runs on the Akka runtime: a xref:sdk:workflows.adoc[Workflow] when the orchestration steps are fixed in code, or an xref:sdk:autonomous-agents.adoc[Autonomous Agent] coordination capability (delegation, handoff, teams, moderation) when the model should decide which agent runs next. Both inherit the same composability, durability, and recovery guarantees from the runtime. The coordination tools that the Autonomous Agent runtime exposes to the model are platform-mediated and durable, and are distinct from the ad-hoc protocol calls to avoid.

For the mechanics of how a supervisor reaches its agents and external services, see xref:sdk:component-and-service-calls.adoc[] and xref:sdk:integrations/index.adoc[Integrations].
...
=== Examples
A workflow step can call two agents concurrently and gather their results before passing them to the next step. See xref:sdk:workflows.adoc[Workflows] for how to build concurrent steps.
