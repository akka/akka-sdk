...
Akka provides three ways to deploy services:

- Offline development with the SDK.
- Self-managed nodes.
- Fully-automated regions.

Akka services can be run in all three modes without code modifications. Akka clustering is provided within all three modes. These deployment modes enable Akka to deploy onto any infrastructure, though some environments block the networking required for Akka clustering to operate.
...
For sleeping peacefully with automated, multi-tenant, and secure operations. You can deploy your services into Akka’s Serverless environment or your VPC with BYOC (managed by Akka) or Self-Hosted (managed by you). Akka regions are powered by Akka Platform, a fully-automated tech stack that manages your Akka services and takes responsibility for SLAs tied to elasticity, agility, and resilience of your Akka services.
...
ifdef::todo[TODO:  [NEED TO ADD LINK]]
==== Removing regions
