...
image:event-sourced-entity-flow.png[Concepts Events Source Flow]
...
The business logic also describes the reply as the commands effect which is passed back to the Endpoint image:steps-6.svg[width=20]. The Endpoint replies to the client when the reply is processed image:steps-1.svg[width=20].
...
image:key-value-entity-flow.png[Concepts Key Value Flow]
