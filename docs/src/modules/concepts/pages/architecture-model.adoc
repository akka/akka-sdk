= Architecture model
:page-summary: How Akka structures a project into api, application, and domain layers, and how those layers map to services.
...
* The `resources` directory includes configuration files and other static content. A `static-resources` subdirectory holds the files of a web application, which the service can serve to a browser alongside its API. See xref:sdk:web-applications.adoc[Web applications].
...
Access control and request validation also belong here, enforced at the service boundary. For HTTP-specific guidance, see xref:sdk:http-endpoints.adoc[Designing HTTP Endpoints].
