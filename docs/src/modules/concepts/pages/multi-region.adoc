...
Read requests are always handled locally within the region that they occur.

An entity can only be written within a single region, known as the primary region.

How Akka assigns the primary region to an entity is configurable. There are two modes for primary selection: **pinned-region** and **request-region**.

In the **pinned-region** primary selection mode (which is the default), the primary region for an entity is selected statically as part of the deployment, so all entities will have the same primary region. Write requests to the primary region of the entity are handled locally. Write requests to other regions are forwarded to the primary region. The primary region stays the same until there is an operational change of the primary region.
...
In the **request-region** primary selection mode, each entity’s primary region will be the region that receives the write request. When a write request arrives to an entity that is not the primary it will be selected as the new primary. The new primary ensures that all preceding events from the previous primary have been fully replicated and applied (i.e. persisted) before writing the new event, and thereby guarantees strong consistency when switching from one region to another. Subsequent write requests to the primary region of the entity are handled locally without any further coordination. Write requests to other regions will trigger the same switch-over process. All other entity instances operate unimpeded during the switch-over process.

This is useful for scenarios where you want to have the primary region for you data close to the users who use the data. A user, Alice, in the USA, will have her data in the USA, while a user Bob, in the UK, will have his data, in the UK. If Alice travels to Asia the data will follow her.
...
=== Example of writes from pinned region
...
Akka guarantees that all events created within one region are eventually replicated to another region.

Each entity’s state is a series of events that are persisted in a local event journal, which acts as the source of events that must be replicated from one region to another. Having a durable, local event journal is the foundation for how Akka can recover an entity’s state after a failure event.

Each event has a sequence number that is validated on the receiving side to guarantee correct ordering and exactly once processing of the events. A replicated event is processed by entities in other regions by having it added to its local event journal. Once added to the local event journal, it can be used to handle read requests in those regions.
