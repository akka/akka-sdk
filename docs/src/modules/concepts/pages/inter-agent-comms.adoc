...
Further, the emergence of https://modelcontextprotocol.io/introduction[MCP], https://a2aprotocol.ai/[A2A], and https://agentcommunicationprotocol.dev/introduction/welcome[ACP] have made it (perhaps too) easy to enable direct communication between agentic services. This has led to many techniques for agent-to-agent communications: some extremely valuable and some deceptively dangerous. While Akka makes it simple to implement any of these techniques, it is worth exploring each of these patterns and anti-patterns in detail.

The following table provides a quick reference as to which communication paths a preferred patterns versus ones you should avoid. Additional details are provided thereafter.
...
|⛔| Agent -> MCP Server -> Agent | Avoid leveraging agent’s innate ability to make invocations to tools that include which can then be used to perform remote communication directly with other agents.

|⛔| Endpoint (API) -> Endpoint (API) | Avoid having the microservice that is backing one API invoke another API directly over whatever protocol is supported by both APIs.
...
The real solution to agent-to-agent communication may seem extreme: _never allow direct agent-to-agent communications_. 
...
So how do we actually communicate with other agents and do it the right way?

== Workflow orchestration

The structure, sequence, scale, and resilience of multi-agent collaborations should be done through the use of Akka workflows. See the https://doc.akka.io/java/workflows.html[official documentation] for guides on general workflow use as well as how to build agentic workflows with sequential, parallel, state-transition, and event-driven logic. Implement Workflows across long-running processes with an unknown stop time by incorporating retries and failure logic within the orchestration. 

== Component client

There is an Akka component client for all of the Akka platform’s components, including agents. While an agent should not use this to talk to other agents, endpoints and workflows will use this to trigger and orchestrate agents.
...
If you need to communicate with an endpoint that is defined _within_ the same project, then Akka has a component client to make this easy. This saves you from having to write code that creates a client that understands the protocol of that endpoint, whether it’s HTTP, gRPC, or MCP.
...
There is no automatic failover or retry or timeout system for when an agent invokes a tool via MCP. This means any agent-to-agent communication done inside of an MCP tool is inherently unreliable and brittle. So, be mindful about using MCP for agent-to-agent calls.
