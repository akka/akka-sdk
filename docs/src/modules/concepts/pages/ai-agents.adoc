...
For more detail on orchestration, check out our xref:concepts:ms-agent-patterns.adoc[agentic orchestration patterns] section.
...
You submit the prompt to the model and get an answer back. Then, we can use _another model_ to derive metrics from that response like confidence ratings. This is often called the "LLM-as-judge" pattern.
