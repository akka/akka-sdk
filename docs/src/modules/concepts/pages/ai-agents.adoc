...
:page-related: xref:concepts:ms-agent-patterns.adoc[], xref:concepts:inter-agent-comms.adoc[], xref:concepts:distributed-systems.adoc[], xref:sdk:autonomous-agents.adoc[]
...
* Compose multiple Agents under platform-mediated coordination: a Workflow supervisor when the orchestration steps are fixed in code, or an xref:sdk:autonomous-agents.adoc[Autonomous Agent] when the model should decide which agent runs next through delegation, handoff, teams, or moderation.
...
Akka offers two forms of supervision, both running on the Akka runtime with the same durable-execution, retry, and audit guarantees. A xref:sdk:workflows.adoc[Workflow] supervises agents from outside, with explicit steps written by the developer; this is the right choice when the orchestration sequence is fixed in code. An xref:sdk:autonomous-agents.adoc[Autonomous Agent] supervises through declared coordination capabilities (delegation, handoff, teams, moderation), with the framework driving the loop and the model deciding which agent runs next; this is the right choice when the orchestration sequence itself is a model judgment.
