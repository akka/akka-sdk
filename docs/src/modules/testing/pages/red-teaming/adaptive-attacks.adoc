= Adaptive attacks
:page-summary: An adaptive attack is a stateful attacker that revises each next prompt based on the target's last reply. Runs inside a durable driver, so a mid-attack crash resumes from the same state.
:page-when-to-use: Use when a target resists the first prompt and the attacker's job is to find a way through. PAIR, Crescendo, and Tree-of-Attacks are the standard techniques `evalkit-redteam` ships.
:page-related: xref:testing:red-teaming/techniques.adoc[], xref:testing:red-teaming/evaluators.adoc[], xref:testing:red-teaming/reports.adoc[]
:page-banner: evaluations

include::ROOT:partial$include.adoc[]

An adaptive attack chooses each next prompt from the target's last reply.
The attacker is a stateful process — every turn reads the reply, decides how to escalate, and produces the next prompt.
...
== Writing a custom adaptive style
...
Every campaign sets an attack budget.
...
== Handling multi-turn attacks in your service
...
Configure `akka.javasdk.agent.<agentId>.guardrails` with a `TurnCountGuardrail` (or a customer implementation) that terminates the session when the count exceeds a threshold.
`crescendo`, `pair`, and `tap` all rely on more turns than a well-behaved user needs.
...
The same `RefusalConsistencyEvaluator` redkit uses in tests can run as a guardrail in production; see xref:sdk:agents/guardrails.adoc[].
...
* Set a token budget on every campaign. Adaptive attacks can loop; budgets are the safety net.
