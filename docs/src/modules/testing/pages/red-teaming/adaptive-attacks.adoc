...
Adaptive attacks choose each next prompt based on the target's last reply.
They run inside `AdversaryDriver`, a turn-by-turn driver that persists its state after every turn.
A JVM restart mid-attack resumes from the same state and produces the same final outcome.
