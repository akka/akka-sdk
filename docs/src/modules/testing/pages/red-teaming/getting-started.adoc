...
`akka-javasdk-redkit` depends on `evalkit`, so the evaluation dependency comes in transitively.
A project with a red-team suite has both trees at test scope.

TIP::
+
--
Alternatively: Add the EvalKit to in the `dependencies` section in your Akka SDK project's Maven `pom.xml` file. +
```xml
  <dependencies>
    <dependency>
