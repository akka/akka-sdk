...
`mvn verify` compiles the classes without running them.
`mvn verify -Dredteam=true` runs them.
...
`evalkit-redteam` depends on `evalkit`, so the evaluation dependency comes in transitively.
...
For the full code, see xref:testing:evaluation/getting-started.adoc[].
...
----
include::example$evaluation-playground/src/test/java/com/example/redteam/campaign/RefundAgentRedTeamCampaign.java[tag=class]
----
...
`RedTeamPanels.render` returns the report as a `String`.
The campaign writes it wherever the project keeps them.
