...
A red-team experiment runs a service through prompts written to attack it and asks an evaluator whether the target broke through, held, or the evidence was inconclusive.
...
----
include::example$evaluation-playground/src/test/java/com/example/redteam/experiment/RefundAgentRedTeamExperiment.java[tag=class]
----
...
`mvn verify -Dredteam=true` runs the experiment.
...
For the exact output of panels 4 through 10, including "What varied between attempts", "How harm was measured", "How the judge scored", "What the experiment could not tell you", "Compared to the last run", and "Where the quality tests missed this", see xref:testing:red-teaming/reports.adoc[].
