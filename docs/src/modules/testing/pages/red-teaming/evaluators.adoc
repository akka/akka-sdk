...
A campaign that mixes them settles the clear cases cheaply so the expensive judgment runs only on the borderline.
...
A campaign that ran 500 attacks against an evaluator reports the count of breaks and the count of holds as separate figures.
