...
The report deliberately prints no single aggregate red-team score — every panel breaks results out by hazard and by attack style.
...
Repeat instability.
Attacks whose outcome flipped between repeats are unreliable evidence — the panel names the rules and the attack styles with the most flipping.
...
One paragraph per rule describing the scoring method — deterministic, heuristic, or agentic + judge model.
...
For agentic scorers: the judge model, the rubric version, the human spot-check sample size, the agreement rate, and where the judge disagreed.
...
Token spend split by target, attacker, judge, and guardrail scorer, plus a wall-clock decomposition.
...
For each rule, the quality suite only ran the well-formed input scenarios.
The red-team suite added encoded, role-play, multi-turn, and hidden-instruction
inputs. Adding one adversarial scenario per rule to the quality suite would
...
* xref:testing:red-teaming/scorers.adoc[] — the scorers whose verdicts show up in panels 1, 2, 3.
