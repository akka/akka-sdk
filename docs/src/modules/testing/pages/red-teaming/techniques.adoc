...
:page-when-to-use: Use when a red-team experiment needs to broaden the attack surface. Encode the payload, roleplay the request, split it across turns, or escalate on a target that resists the first prompt.
:page-related: xref:testing:red-teaming/adaptive-attacks.adoc[], xref:testing:red-teaming/evaluators.adoc[], xref:testing:red-teaming/experiments.adoc[]
