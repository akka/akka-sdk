...
:page-summary: A red-team experiment runs a set of rules against a target under a set of attack styles. Every attempt is one eval case, one rule, and one attack style, repeated as many times as the experiment asks for.
:page-when-to-use: Use to schedule a red-team run (nightly, on every release, or on demand) with a stated token budget and a hazard set the experiment speaks to.
...
A red-team experiment is a named unit of eval cases, rules, and attack styles that run together.
...
The command wraps the Maven invocation that runs every experiment under `src/test/java/com/example/redteam/`.
