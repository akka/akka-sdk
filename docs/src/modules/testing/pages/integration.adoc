...
`LedgerClient` is available in tests.
Fetch the interaction record an evaluator would fetch and assert on its fields.
Useful when verifying that an agent recorded what it was expected to.
...
| Ledger read
| `LedgerClient` injected into the test

| An evaluator wired to a real agent
| Existing `EvaluatorIntegrationTest` pattern under `akka-javasdk-tests`
|===
