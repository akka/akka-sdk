...
:page-summary: How the pieces of an evaluation suite fit together. The customer writes cases and a target adapter, an experiment setup collects them, the runner walks the cases, and evaluators produce one result per attempt.
:page-related: xref:testing:evaluation/eval-cases-and-rules.adoc[], xref:testing:evaluation/evaluators.adoc[], xref:testing:evaluation/campaigns.adoc[]
...
`SystemUnderTest`::
The service being evaluated. Two methods: `prepare(EvalSetup)` puts it into the state a case names; `submit(sessionId, userText)` speaks the graded turn.

`Evaluator`::
A check over one recorded attempt. Reads an `EvaluationContext` and returns an `EvaluationResult`. Three families ship: deterministic, heuristic, agentic.

`ExperimentSetup`::
A named set of eval cases plus the rubric, lanes, policy, and repeat count. What to run.

`ExperimentRunner`::
Walks the setup, calls the adapter, hands each attempt to an evaluator, and returns one `EvalCaseResults` per case plus an `ExperimentReport` counting the whole run.

Campaign::
The JUnit test class that assembles the pieces above and calls `ExperimentRunner.run(...)`. Not an SDK class. A naming convention for the `*Campaign.java` files. Gated on `-Deval=true`.
...
flowchart TB
    ...
    class Result,Results,Report results
----

Blue is what you implement.
...
| Campaign               | Your JUnit test class. Convention, not an SDK type.
| `RemoteCampaign`       | Wire client for dispatching a run to the offline-evals service.
...
* xref:testing:evaluation/getting-started.adoc[]. Walks the concepts end-to-end.
* xref:testing:evaluation/eval-cases-and-rules.adoc[]. What an eval case declares in full.
* xref:testing:evaluation/evaluators.adoc[]. The three families and how to add your own.
* xref:testing:evaluation/campaigns.adoc[]. Building an experiment setup and running it.
