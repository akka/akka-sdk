...
<2> A `List<EvalCase>`.
Read them from wherever the project keeps them.
`EvalSource` is the interface for paging a large set, and a project supplies its own implementation.
...
`Check.Refused` carrying the reasons.
A campaign naming a fixture the target cannot build is refused in seconds rather than yielding one `NotReached` per case at the end of a long run.
...
A campaign over `RecordedInteractions` scores runs evalkit did not execute.
...
* Keep the eval case count per campaign small enough that a local run finishes quickly.
