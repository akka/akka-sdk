...
| `setup`
| A `Runnable` that runs before the agent is called. Use it to load answers into the stubs the agent's tools call, or to seed entities through the component client. `EvalCase.NO_SETUP` when the case needs none. `EvalCase.of(id, userMessage, evaluators...)` creates a case without a setup.
