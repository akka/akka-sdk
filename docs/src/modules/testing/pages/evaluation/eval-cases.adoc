...
:page-summary: An EvalCase carries the command sent to the agent and its evaluators; the built-in evaluators check the tool calls, the reply and the spend of a turn, and are inconclusive when their evidence is absent.
...
| `recordedCalls`
| The tool calls a recording carried, as data. The runner loads their results into the stubs through the `ToolBindings` given to it, see xref:testing:evaluation/replay.adoc#tool-bindings[Binding tools to stubs]. Empty for a case written by hand. `EvalCase.of(id, command, evaluators...)` creates such a case.
...
A judge and the report read the command as text: a `String` command as is, any other command as its JSON.
`EvalCase.commandText()` returns that text.
