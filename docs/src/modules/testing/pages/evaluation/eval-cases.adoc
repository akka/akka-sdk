...
A case that throws while its recorded calls are loaded is reported as failed under the `setup` evaluator.
...
Every evaluator class carries an `@EvalLabel`, the name the report prints its results under.
A built-in evaluator reports under its label as is, for example `tools`.
A custom evaluator reports under `custom-eval:` followed by its label, so a check of your own never merges into a built-in row of the report.
A case refuses an evaluator whose class has no `@EvalLabel`.

`Gate.evaluatorRateAtLeast` takes the evaluator's class, for example `Evaluators.ToolArgument.class` or `RefundWithinTotal.class`, and reads the label from it.
...
A custom evaluator is a named class, annotated with `@EvalLabel`, that implements link:{attachmentsdir}/testkit/akka/javasdk/testkit/eval/Evaluator.html[Evaluator].
`evaluate(...)` receives the case and the `Interaction`, the reply and everything the runtime traced for the turn, and returns an `EvalResult`: a pass, a fail with a reason, or an inconclusive result.
The report prints the results under `custom-eval:` and the label, see <<evaluator-names>>.
...
----
<1> The label. The report prints the results under `custom-eval:refund-within-total`, and `Gate.evaluatorRateAtLeast(RefundWithinTotal.class, rate)` rates them.
