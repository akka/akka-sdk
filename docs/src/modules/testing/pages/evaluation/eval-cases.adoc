...
A built-in evaluator reports under the name its constant in `Evaluators` gives, for example `tools`.
A custom evaluator reports under `custom-eval:` followed by the name it returns from `name()`.
The runner adds the prefix, so a check of your own never merges into a built-in row of the report.
...
The runner prints the results under `custom-eval:` and that name, see <<evaluator-names>>.
