...
:page-when-to-use: Use when an experiment runs on a schedule or in CI, and its purpose is to catch regressions between the intended-good target and the target on this build.
:page-related: xref:testing:evaluation/experiments.adoc[], xref:testing:evaluation/reports.adoc[]
...
== How an experiment compares against a baseline
...
*One baseline per experiment name.*
