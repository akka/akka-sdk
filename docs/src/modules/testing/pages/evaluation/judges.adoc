...
What the agent was asked is the user message the agent sent the model, read from the runtime trace.
That is the text the model under test saw, which is not always the case's command: a command handler builds the user message from the command, and may wrap it or leave fields of it out.
When the trace carries no user message, the case's command is used instead.

`ModelBasedJudge` renders the sections as text and sends it as the user message.
