...
A link:{attachmentsdir}/testkit/akka/javasdk/testkit/eval/Judge.html[Judge] scores such a criterion between 0 and 1, and the score becomes a result on the case with the label `judge`.
...
`Gate.passRateShouldBeAtLeast(Evaluators.JudgeEvaluator.class, minimumRate)` rates the `judge` evaluator over the cases where it was conclusive, and fails when it judged no case.
