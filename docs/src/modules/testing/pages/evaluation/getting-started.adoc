...
A campaign is one class in the suite.
...
Campaigns are gated on `-Deval=true` so `mvn verify` compiles the classes without running them.
`mvn verify -Deval=true` runs them.
...
In your Akka SDK project's Maven `pom.xml` file. Add the EvalKit to in the `dependencies` section:
```xml
  <depedencies>
    <dependency>
...
----
include::example$evaluation-playground/src/test/java/com/example/eval/runner/RefundAgentRunner.java[tag=class]
----
...
A campaign that names a fixture this adapter never declared is refused before running.
...
----
include::example$evaluation-playground/src/test/java/com/example/eval/campaign/RefundPolicyCampaign.java[tag=class]
----
...
`Panels.render(result.asRecord(setup, reporting))` returns the report as a `String`.
