...
:page-related: xref:testing:evaluation/evaluators.adoc[], xref:testing:evaluation/eval-cases-and-rules.adoc[], xref:testing:evaluation/experiments.adoc[]
...
Experiments are gated on `-Deval=true` so `mvn verify` compiles the classes without running them.
...
----
include::example$evaluation-playground/src/test/java/com/example/eval/experiment/RefundPolicyExperiment.java[tag=class]
----
...
`mvn verify -Deval=true` runs the experiment.
...
* xref:testing:evaluation/experiments.adoc[]. Building an experiment setup, and rendering a report from a run.
