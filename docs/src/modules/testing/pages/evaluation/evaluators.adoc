...
:page-related: xref:testing:evaluation/index.adoc[], xref:testing:evaluation/eval-cases-and-rules.adoc[], xref:testing:evaluation/experiments.adoc[], xref:testing:evaluation/reports.adoc[], xref:testing:red-teaming/evaluators.adoc[]
...
Each result carries an `EvaluationOutcome`: `VERDICT` (a pass or fail with an optional score), `UNDECIDED` (the evaluator looked and declined to call it), `INCONCLUSIVE` (the evaluator had nothing to read), or `FAILED` (the evaluator itself broke).
...
----
include::example$evaluation-playground/src/test/java/com/example/eval/evaluator/deterministic/ToolPermissionSample.java[tag=evaluator]
----
...
.ToolPermissionSample.java
----
include::example$evaluation-playground/src/test/java/com/example/eval/evaluator/deterministic/ToolPermissionSample.java[tag=strict]
----
...
.ToolPermissionSample.java
----
include::example$evaluation-playground/src/test/java/com/example/eval/evaluator/deterministic/ToolPermissionSample.java[tag=deny]
----
...
----
include::example$evaluation-playground/src/test/java/com/example/eval/evaluator/heuristic/LatencyBudgetSample.java[tag=evaluator]
----
...
.LatencyBudgetSample.java
----
include::example$evaluation-playground/src/test/java/com/example/eval/evaluator/heuristic/LatencyBudgetSample.java[tag=interpret]
----
...
----
include::example$evaluation-playground/src/test/java/com/example/eval/evaluator/agentic/TurnFaithfulnessSample.java[tag=wiring]
----
...
Check that there is anything to ask about, call the assessor, validate the returned score is a share in `[0, 1]`, produce an `EvaluationResult`.
`EvalContext.asContext()` turns the customer's evidence into the shared context every evaluator reads.
...
.TurnFaithfulnessSample.java
----
include::example$evaluation-playground/src/test/java/com/example/eval/evaluator/agentic/TurnFaithfulnessSample.java[tag=stub-for-tests]
----
...
----
include::example$evaluation-playground/src/test/java/com/example/eval/evaluator/deterministic/RefundWindowEvaluator.java[tag=class]
----
...
For the reference tables listing every built-in evaluator and what each reads, see xref:reference:evaluations/evaluators.adoc[].
...
* xref:testing:evaluation/getting-started.adoc[]. A worked example from empty project to first experiment.
* xref:testing:evaluation/eval-cases-and-rules.adoc[]. What an eval case declares, and which evaluator settles it.
* xref:testing:evaluation/experiments.adoc[]. How a set of eval cases reaches a target.
