...
| At least this share of the named evaluator's results passed. Inconclusive results are left out of the count. The gate fails when the evaluator has no result to count. The name is a constant in `Evaluators`, or the name a custom evaluator returns from `name()`.
