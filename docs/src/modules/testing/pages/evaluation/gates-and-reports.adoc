...
| `Gate.evaluatorRateAtLeast(evaluatorClass, rate)`
| At least this share of the evaluator's results passed. Inconclusive results are left out of the count. The
gate fails when the evaluator has no result to count. The class is a built-in nested in `Evaluators`, for example
`Evaluators.ToolArgument.class`, or your own evaluator class.
...
. One line per evaluator, under its xref:testing:evaluation/eval-cases.adoc#evaluator-names[evaluator name]: the cases it passed over the cases it judged. An evaluator that was inconclusive on some cases shows the count in brackets, for example `token-budget 0/0 (2 inconclusive)`.
