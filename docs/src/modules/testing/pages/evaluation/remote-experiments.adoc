= Remote experiments
:page-summary: A remote experiment runs on the offline-evals service instead of the customer's JVM. The setup the customer writes and the report the customer reads back are the same either way. The service does the work.
:page-when-to-use: Use for datasets over a few thousand eval cases, experiments that take more than a few minutes, experiments whose cost belongs to a shared budget, and scheduled runs.
