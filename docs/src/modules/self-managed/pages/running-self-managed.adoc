= Running Akka with self-managed nodes
...
You can retrieve a free license key from https://account.akka.io/key[akka.io, window="new"]. Select the Akka SDK option.
...
When running more than one instance of the service they must form an Akka Cluster. This is done automatically in a Kubernetes environment. Many container platforms are not designed to support cluster-based systems that need discovery and network connectivity to enable scale and resilience. If clustering is not an option in your environment you can run it as a single instance by defining the following configuration in your `application.conf`:
