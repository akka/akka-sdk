...
* Akka SDK 3.2.4
  - Support for https://doc.akka.io/self-managed/index.html[self-managed nodes] in your infrastructure, or any cloud service. For organizations that want control over how Akka services are installed, updated, and maintained. Akka services are packaged into standalone binaries with Akka clustering for scaling and deploy to self-managed nodes. You are responsible for separately managing secure connectivity, routes, installation, deployment, and persistence.
