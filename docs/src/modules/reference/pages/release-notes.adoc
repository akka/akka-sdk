...
* Akka CLI 3.0.76
- commands to manage snapshots for link:reference:cli/akka-cli/akka_local_services_components_entities.adoc[entities] and link:reference:cli/akka-cli/akka_local_services_components_workflows.adoc[workflows]
+
[source, shell]
----
akka service components entities  delete-snapshot SERVICE_NAME COMPONENT_ID ENTITY_ID
akka service components entities  force-snapshot  SERVICE_NAME COMPONENT_ID ENTITY_ID
akka service components workflows delete-snapshot SERVICE_NAME COMPONENT_ID WORKFLOW_ID
akka service components workflows force-snapshot  SERVICE_NAME COMPONENT_ID WORKFLOW_ID
----
- reorganized commands per component type e.g. `akka service components entities`
- internal improvements


* https://github.com/akka/akka-sdk/releases/tag/v3.6.4[Akka SDK 3.6.4]
- Runtime 1.6.16
- Bedrock model provider option to place the prompt cache point after the last user message
- Testkit runtime port improvements

* xref:release-notes/runtime/1.6.17.adoc[Akka Runtime 1.6.17]
- Optimization of views and consumers that catch up over a long event backlog, and debug tracing fixes

* Akka CLI 3.0.75
- support for more AI assistants in `akka specify init --agent ...`

* Akka CLI 3.0.74
- Network policy support
- Support for Google's Antigravity
- Fix for resolved default in instance-types list
- Fix for nil pointer on regional resource export
- Workload identity fixes

* xref:release-notes/runtime/1.6.16.adoc[Akka Runtime 1.6.16]
...
* Akka CLI 3.0.73
- Maintenance updates
- Fix for service config
- Data management updates
- Improvement to log exports
...
- Log level management improvements

* Akka SDK 3.6.3
- Runtime 1.6.15
- Alpine base image 3.24.1
- Improved error message for deserialization failures
- Unhandled exception reporter covering consumers and timed actions


* Akka CLI 3.0.70
- Enhancements to Akka Specify
    - Exit conditions
    - Definition of Done
    - Enforced mode vs. à-la-carte mode

* https://github.com/akka/akka-sdk/releases/tag/v3.6.2[Akka SDK 3.6.2]
- Dependency updates

== July 2026

* https://github.com/akka/akka-sdk/releases/tag/v3.6.1[Akka SDK 3.6.1]
- Function tools can return images and PDFs to the model
- Function tools can return multiple contents in a single tool result
- Command handlers, queries, and HTTP and MCP endpoint methods are discovered from base classes
- Generic arrays are supported in the generated JSON schema
- TestKit generates the gRPC endpoint classes it implements
- Runtime 1.6.13

* Akka CLI 3.0.66
- Dependency updates
- Added support for store pinning for enabled Organization
...
The new xref:sdk:autonomous-agents.adoc[Autonomous Agent] is an AI model-driven component that runs as a durable process. It works on typed tasks, each with its own identity, instructions, and result schema, and the runtime drives the model through a decision loop until each task is complete. Agent and task state are persisted along the way, so work survives crashes and restarts. A task's status and result can be queried while it runs or after it finishes.

Multi-agent coordination capabilities are built in. An agent can delegate subtasks to specialist workers, hand off work to a peer, lead a team that shares a task list, or moderate a turn-taking conversation. Those capabilities are exposed to the model as tools so multi-agent systems can be assembled without orchestration code. This complements deterministic, xref:sdk:agents/orchestrating.adoc[workflow-based multi-agent orchestration]: pick a workflow when the sequence of steps is fixed in code, and an Autonomous Agent when the model decides which agent runs next. Both run on the same durable-execution, retry, and audit foundation.

****

* https://github.com/akka/akka-sdk/releases/tag/v3.6.0[Akka SDK 3.6.0]
- Autonomous agents
...
* https://github.com/akka/akka-sdk/releases/tag/v3.5.19[Akka SDK 3.5.19]
...
- JWKS configuration support

* https://github.com/akka/akka-sdk/releases/tag/v3.5.18[Akka SDK 3.5.18]
...
* https://github.com/akka/akka-sdk/releases/tag/v3.5.17[Akka SDK 3.5.17]
...
* https://github.com/akka/akka-sdk/releases/tag/v3.5.16[Akka SDK 3.5.16]
...
* https://github.com/akka/akka-sdk/releases/tag/v3.5.15[Akka SDK 3.5.15]
...
Views and consumers can now be configured to xref:sdk:consuming-producing.adoc#_starting_from_snapshot[start from a snapshot] instead of replaying the full event history. This significantly reduces startup time and resource usage for components that subscribe to high-volume event streams.
...
* https://github.com/akka/akka-sdk/releases/tag/v3.5.14[Akka SDK 3.5.14]
...
Agents gain xref:sdk:agents/prompt.adoc#_multimodal_user_message[multimodal capabilities with image input support], enabling models to perceive and reason about visual content. Supported models can now be configured to enable thinking, letting agents reason through complex problems step by step. Other Akka components can be wired directly as agent tools, making it easy to give agents access to your existing business logic without writing glue code.

**Workflow notifications and a new fluent API**

Workflows get xref:sdk:workflows.adoc[a redesigned, more type-safe and fluent API] that makes defining multi-step processes more intuitive. Workflows can now xref:sdk:workflows.adoc#_notification[emit streams of real-time notifications] to subscribers, keeping clients and downstream systems informed as individual workflows progress through their steps.

**Console with deep observability**

The Akka console now provides detailed insights into your service structure and component interactions. xref:sdk:running-locally.adoc#_local_console[Run it in local mode] to get full request traces showing exactly which components handle each request, along with a detailed view of how agents interact with AI models — invaluable for debugging and understanding agent behavior during development.

****

* https://github.com/akka/akka-sdk/releases/tag/v3.5.13[Akka SDK 3.5.13]
...
* https://github.com/akka/akka-sdk/releases/tag/v3.5.12[Akka SDK 3.5.12]
...
* https://github.com/akka/akka-sdk/releases/tag/v3.5.11[Akka SDK 3.5.11]
- resumable SSE for streaming updates view queries
- workflow global timeout and pause timeout configuration

* https://github.com/akka/akka-sdk/releases/tag/v3.5.10[Akka SDK 3.5.10]
