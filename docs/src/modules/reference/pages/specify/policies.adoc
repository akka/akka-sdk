...
* the *ship steps* — the actions the organization runs on a successful ship and the conditions that must be green before those actions execute (see xref:sdk:enforced-mode.adoc#shipping[Shipping and receipts]);
...
ship:                          # org-authored ship steps that run on a successful ship
  steps:
...
. Materialize the result: the MCP configuration and the resolved conditions with their lock state, on/off, and provenance, ready for `status`, xref:reference:specify/conform.adoc[`conform`], and `ship`.
...
* every conformance check — `akka specify status`, xref:reference:specify/conform.adoc[`conform`], and `ship`, plus the `akka_ec_*` MCP tools — applies the policy *by default*, with no flag to pass;
