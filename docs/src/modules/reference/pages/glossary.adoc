...
[[auditor]]Auditor:: The executable check bound to an <<exit_condition>> that evaluates it and returns a verdict. Requirement-aware — it defers when it cannot run in the current environment. See xref:sdk:enforced-mode.adoc#auditors[Auditors].
...
[[governance_level]]Governance level:: The setting an organization assigns to a governed <<exit_condition>>, controlling its default state and whether a developer can change it: always-apply, never-apply, on-but-dev-configurable, or off-but-dev-configurable. See xref:reference:governance/policy-as-code.adoc[].
