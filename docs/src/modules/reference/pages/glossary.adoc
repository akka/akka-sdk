...
[[auditor]]Auditor:: The check bound to an <<exit_condition>> that proves the condition's invariant holds. Most auditors are _declarative_ and delegate to a tool the ecosystem already provides -- a type-checker, test-runner, or linter -- and are green when that command exits zero, red otherwise. There is no provided-auditor repository; a coverage gap is closed by authoring a declarative check. See xref:reference:specify/auditors.adoc[Auditors].
