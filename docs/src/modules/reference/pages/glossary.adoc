...
[[akka_specify_plugin]]Akka Specify Plugin:: The command-line and marketplace tool that runs <<sdd,Spec-Driven Development>> from an AI coding assistant. It installs and configures the toolchain and drives the `specify`, `clarify`, `plan`, `tasks`, `implement`, `build`, and `deploy` commands.

[[blueprint]]Blueprint:: A recipe that the <<akka_specify_plugin>> turns into buildable, runnable AI systems.
