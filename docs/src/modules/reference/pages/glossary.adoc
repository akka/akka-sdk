...
[[definition_of_done]]Definition of Done:: The business statement of when a feature is done, authored in the project owner's terms. Each item is translated into one or more <<exit_condition,exit conditions>>. See xref:sdk:exit-conditions.adoc[].

[[exit_condition]]Exit condition:: The technical unit of "done" in <<sdd,Spec-Driven Development>> — a precise, binary pass/fail predicate over an observable surface (an endpoint response, entity state, a build result), backed by an <<auditor>>. See xref:sdk:exit-conditions.adoc[].

[[auditor]]Auditor:: The executable check bound to an <<exit_condition>> that evaluates it and returns a verdict. Requirement-aware — it defers when it cannot run in the current environment. See xref:sdk:auditors.adoc[].

[[conformance_mode]]Conformance mode:: Whether the <<definition_of_done>> is a hard gate (Enforced) or an advisory checklist (À la carte). See xref:sdk:conformance-modes.adoc[].
...
[[conformance_receipt]]Conformance receipt:: The record written when a feature ships, listing which <<exit_condition,exit conditions>> were verified, which deferred downstream, and any waivers in effect. See xref:sdk:shipping-and-receipts.adoc[].
