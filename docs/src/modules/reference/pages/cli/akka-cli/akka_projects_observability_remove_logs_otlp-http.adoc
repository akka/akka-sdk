= akka projects observability remove logs otlp-http

Remove an OTLP HTTP exporter from logs by endpoint.

----
akka projects observability remove logs otlp-http [flags]
----
