= akka projects observability remove traces otlp-http

Remove an OTLP HTTP exporter from traces by endpoint.

----
akka projects observability remove traces otlp-http [flags]
----
