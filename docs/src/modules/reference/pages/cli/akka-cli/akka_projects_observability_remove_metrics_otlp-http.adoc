= akka projects observability remove metrics otlp-http

Remove an OTLP HTTP exporter from metrics by endpoint.

----
akka projects observability remove metrics otlp-http [flags]
----
