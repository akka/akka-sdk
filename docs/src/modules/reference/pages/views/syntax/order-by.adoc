...
`ORDER BY` cannot be used together with token-based pagination (`next_page_token()` / `page_token_offset()`), since that form of pagination relies on a fixed, implicit ordering. It can be freely combined with count-based pagination (`OFFSET`/`LIMIT`) — see xref:reference:views/concepts/pagination.adoc[Pagination].
