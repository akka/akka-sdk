...
NOTE: `page_token_offset()` cannot be combined with `ORDER BY`. Results are implicitly ordered by the row's position (id) so the paging cursor stays consistent between requests. For custom sorting, use count-based pagination (numeric `OFFSET`/`LIMIT`) instead.
