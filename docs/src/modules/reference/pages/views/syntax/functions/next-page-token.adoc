...
NOTE: `ORDER BY` cannot be combined with `next_page_token()` / `page_token_offset()`. Results are implicitly ordered by the row's position (id) so that the paging cursor stays consistent. If you need custom sorting with pagination, use count-based pagination (`OFFSET`/`LIMIT`) instead — see xref:reference:views/syntax/order-by.adoc[ORDER BY].
