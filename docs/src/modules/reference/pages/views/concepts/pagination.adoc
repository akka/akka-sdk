...
NOTE: This only applies to count-based pagination. `ORDER BY` cannot be combined with token-based pagination (see <<Token-based Pagination>>) — the token cursor relies on a fixed, implicit ordering.
