= Descriptors

include::ROOT:partial$include.adoc[]

Descriptors support structured configuration across multiple Akka regions and Akka projects. They are applied through the Akka CLI.

* The xref:descriptors/project-descriptor.adoc[Project descriptor] defines the complete configuration for an Akka project
and may even be used to create a new project.
* The xref:descriptors/service-descriptor.adoc[Service descriptor] defines the configuration for a service resource.
* The xref:descriptors/route-descriptor.adoc[Route descriptor] describes the way ingress traffic is routed to your Akka services.
* Through the xref:descriptors/external-secret-descriptor.adoc[External secret descriptor] an Akka service is configured
to source secrets from an external secret manager.
* The xref:descriptors/observability-descriptor.adoc[Observability descriptor] describes how metrics, logs,
and traces are exported to third party services.
* The xref:descriptors/model-descriptor.adoc[Model descriptor] declares the models an inference cluster serves,
the hardware they run on, and the hostnames that expose them.
* The xref:descriptors/routing-ruleset.adoc[Routing ruleset] declares the rules the gateway router matches
against request attributes to choose a model provider.
