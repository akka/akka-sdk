...
A _Project Descriptor_ in Akka is a multi-document YAML file that defines the complete configuration for an Akka project, including services, routes, observability settings, message broker configuration, and role bindings. This allows you to manage multiple resources in a single file and apply them atomically with the `akka project apply` command.
...
The project descriptor supports the following resource types. A descriptor can contain multiple `Service` and `Route` documents, each with a unique name. The `Observability`, `Broker`, and `RoleBindings` resources are project-level singletons—only one of each can exist per project.
...
Configures the project's OIDC group-to-role mappings. Only one role bindings configuration exists per project, but it can contain multiple group-to-role mappings. This enables automated role assignment based on group membership when using OIDC authentication at the organization level. The project must be owned by an organization with OIDC configured.
...
This exports all services, routes, observability configuration, broker settings, and role bindings to a single file. You can also write to stdout:
...
To validate a descriptor and see what would be applied without making changes:

[source,bash]
----
akka project apply -f project.yaml --dry-run
----
...
[source,bash]
----
cat project.yaml | akka project apply -f -
----
