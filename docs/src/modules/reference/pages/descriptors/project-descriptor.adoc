...
|*resource*         |string _required_  |The type of resource: `Service`, `Route`, `Observability`, `ExternalSecret`, `Broker`, or `RoleBindings`
|*resourceVersion*  |string _required_  |The version of the resource schema. Currently only `v1` is supported.
|*metadata*         |<<Metadata>>       |Optional metadata for the resource. Required for `Service`, `Route` and `ExternalSecret` resources.
...
|*name*  |string   |The name of the resource. Required for `Service`, `Route` and `ExternalSecret` resources.
|===

== Resource types

The project descriptor supports the following resource types. A single descriptor can contain multiple `Service`, `Route` and `ExternalSecret` documents, each with a unique name. The `Observability`, `Broker`, and `RoleBindings` resources are project-level singletons—only one of each can exist per project.
