...
Setup depends on two choices: which cloud you run in and which deployment model you use.

By deployment model:

* *Dedicated* — Akka hosts and operates the region in Akka's cloud, so there are no customer installation steps. Akka provisions the region and connects it to you over a private link. See xref:operations:production-readiness/dedicated/checklist.adoc[Dedicated production readiness].
* *BYOC* — Akka provisions and operates a region inside your own cloud account. Start with xref:operations:technical-overview/byoc.adoc[Bring Your Own Cloud setup] (install flow, `akka-bootstrap`, identity model), then your cloud page below.
* *BYOK8s* — you provision the Kubernetes cluster to Akka's specification. Start with xref:operations:technical-overview/byok8s-requirements.adoc[BYOK8s requirements], then your cloud page below.

By cloud (BYOC and BYOK8s):
