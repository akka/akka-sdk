...
Akka Automated Operations (AAO) is a remotely installed, actively maintained Akka region that runs inside your own environment, on Kubernetes across AWS, Azure, or GCP. You keep custodial ownership of the infrastructure. Akka installs, monitors, patches, and scales it for extreme availability and low latency.
...
[NOTE]
====
Download the full overview as a single link:_attachments/whitepapers/aao-technical-overview-full.pdf[PDF, window=_blank]. Each page below is also available as its own PDF.
====
...
.The Federation Plane and one or more per-region Application Planes
