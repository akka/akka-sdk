...
| *Dedicated*
| Akka hosts and operates the region in Akka's cloud. There are no customer installation steps.
| xref:operations:production-readiness/dedicated/checklist.adoc[Dedicated readiness]

| *BYOC*
| Akka provisions and operates a region inside your own cloud account, using the `akka-bootstrap` install flow and identity model.
| xref:operations:technical-overview/byoc.adoc[BYOC setup]

| *BYOK8s*
| You provision the Kubernetes cluster to Akka's specification, then Akka installs the platform into it.
