...
| Networking | Ingress/Egress Connectivity using private patterns with auto autogenerated IP address | A | A | A | C | Akka provisions the load balancer for the region with an automatically generated IP address, ensuring internal communication without traversing the public internet. | 
...
| Networking | IP Allowlist | C | C | A | A | The Customer is responsible for setting up and maintaining the IP allowlist for NAT gateways and load balancer. The Customer informs Akka of changes that may affect Platform connectivity. | 
...
| Federation | Teleport identity-aware proxy | C | C | A | A | The Customer installs and maintains the Teleport identity-aware proxy used for federation, including ongoing operational health and version upgrades. Akka provides a short-lived token and the installation command at setup time. | 
...
| Setup | Provide maintenance window, usually 1-4 hours per week. | C | C | A | A | The maintenance window is used by Akka for any operation that may cause downtime. | 
| Maintenance | Infrastructure patch / Vulnerability management | A | A |  | C | Akka manages infrastructure patching and vulnerability remediation for the Platform, including the JVM, the container base image, and the Akka runtime that ship with the customer's deployed services. Vulnerability reports are available from Akka's trust center. | 
| Maintenance | K8s / DB Upgrades | C | C | A | A | The Customer schedules and runs Kubernetes and database upgrades. The Customer must consult Akka in advance so that Akka can certify the target versions against the Platform. Running an unverified version may break Platform functionality and is the Customer's responsibility. | 
...
| Operations | Scalability (Infrastructure) | C | C |  | A | Customer is responsible for ensuring scalability for the platform; includes compute capacity availability for node pools and database scalability | 
...
| Observability | Akka Region Platform Telemetry for the Customer | C | C | A |  | Customer can optionally choose to set up platform telemetry for the cluster and for the database instance as they own the account. Akka can be consulted for components that they can set up observability | 
...
| Backups and Recovery | Database Backups (Automated recurring) | C | C |  | A | Customer is responsible for configuring recurring database backups | 8-day retention
...
| Backups and Recovery | Velero used for K8s resources | ± | ± |  |  | Akka uses Velero to back up stateful Kubernetes resources for point-in-time recovery. Customer sets up necessary IAM binding and object storage for Akka | 
| Backups and Recovery | Backup Restore Testing | ± | ± | C |  | Akka performs periodic restore testing against backups; results are summarized on request. The Disaster Recovery Exercise row covers joint exercises. | Quarterly
...
| Permissions | Quarterly Access-Control Reports | A | A |  |  | Akka delivers quarterly access-control reports covering Akka's own privileged access to the Customer's installation. The Customer manages their own IAM independently. | Quarterly
