...
Akka:: Installs the platform machinery (Akka runtime, operators, registry, observability) into the Customer's cluster and configures region groups.
Customer:: Provisions the cluster and database, installs Teleport for federation using the short-lived token Akka provides, identifies region groupings, and provides a recurring 1 to 4 hour weekly maintenance window that Akka uses for any operation that may cause downtime.
...
| Platform bootstrapping | Akka | Akka installs the platform machinery (runtime, operators, registry, observability) into the Customer's Kubernetes cluster after the Customer provisions the underlying infrastructure.
...
| Application telemetry export | Customer | Customer configures export of application logs, metrics, and traces to their observability platform via the Akka SDK integration patterns.
|===
====

== Backups, disaster recovery, and multi-region

The Customer runs database backups on their infrastructure; Akka uses Velero for Kubernetes resource backups with infrastructure the Customer provides. Restore testing, disaster-recovery exercises, and replication topology are joint decisions, and Akka provides multi-region failover tooling.

Akka:: Operates Velero for K8s resource backups (against Customer-provided IAM and object storage), provides multi-region failover tooling and runbooks, publishes default RTO/RPO.
Customer:: Configures recurring database backups, notifies Akka after ad-hoc backups, specifies replication mode per service, is consulted before failover.
...
| Automated database backups | Customer | Customer configures recurring database backups in their environment. Akka is informed. *8-day retention recommended.*
...
| Kubernetes resource backups (Velero) | Shared | Akka uses Velero to back up stateful Kubernetes resources for point-in-time recovery. Customer provides the necessary IAM binding and object storage.
| Backup restore testing | Shared | Akka and the Customer perform periodic restore testing against backups; results are summarized on request. The Disaster Recovery Exercise row covers joint exercises. *Quarterly.*
...
Akka:: Uses Teleport for session-recorded privileged access and delivers quarterly access-control reports. Operates within the service-account access the Customer grants.
...
| Quarterly access-control reports | Akka | Akka delivers *quarterly* access-control reports covering Akka's own privileged access to the Customer's installation. The Customer manages their own IAM independently.
|===
====
...
Shared:: BYOK8s onboarding, support & communication channels, customer portal training, end-of-contract decommission within 30 days.
...
| Customer portal access & training | Shared | Customer signs up for portal access; Akka assigns the region and walks the Customer through console, CLI, and ticketing flows.
| Customer offboarding & data return | Shared | At end of contract, Akka coordinates region decommission, return or destruction of customer data, and revocation of access per the Data Retention Policy. *Standard cadence: 30 days from termination.*
...
| Personnel security & background checks | Akka | Akka performs background checks on personnel with access to customer environments and provides security awareness training, per the Personnel Security Policy. *Annual access certification.*
...
Customer-specific targets may be agreed where the standard does not meet the Customer's needs. Compliance posture (SOC 2 Type II, annual third-party penetration test, current subprocessor list) is published on trust.akka.io.
====
...
| RTO / RPO | Recovery Time Objective / Recovery Point Objective. Default platform commitments are in the Backups section and Appendix B.
