...
What Akka expects of customer applications before going live in BYOK8s (Bring Your Own Kubernetes), the deployment model where Akka runs inside a Kubernetes cluster the customer provides and operates. Akka handles most of the operational surface and protects the deployment within those bounds. Production is still a shared outcome; the items below are the customer's side of that line.
...
This page is a guideline. The authoritative checklist for your application is the one you build with your Akka team.

Walk this page before going live: use the at-a-glance below as the quick entry point and the detailed checklists for specifics. The items are a baseline drawn from BYOK8s deployments; add anything specific to your request flows and failure modes. Discuss modifications or removals with your Forward Deployed Engineer (FDE), the named Akka engineer assigned to your account, so the original rationale is shared and the change is evaluated jointly.
...
Each item is yours to verify. Akka recommends not advancing to production until each is covered. Engage your FDE and Akka's customer success and support teams (mailto:support@akka.io[support@akka.io]) throughout the work, or complete it independently.
...
In BYOK8s the customer provides and operates the Kubernetes cluster, the database, and the underlying cloud infrastructure. Akka runs the platform end-to-end inside that cluster: runtime, region installation, scaling, patching of platform components, platform observability, and the operational rituals around them. The items on this page are the customer-side complement. Your FDE walks the principal items before go-live.
...
.*Akka's side*
====
Akka runs a parallel internal pre-production validation against the platform-side commitments in the RACI: replication, observability stack, baseline regional alerts, certificate monitoring. Database backups sit on the customer side in BYOK8s and are covered in the items below rather than in Akka's parallel validation. This page is the customer-side complement; the two pair together.
====
...
Start with a tabletop: walk failure scenarios end-to-end, confirm who does what, capture prerequisites and gaps. Then exercise the scenarios that matter most live. The xref:operations:production-readiness/byok8s/disaster-recovery.adoc[disaster recovery scenario catalog] is the detailed reference the tabletop walks through.
...
Where you are regulated, these are the items auditors and regulators ask for. Consider each; skip only with a documented reason. In BYOK8s more of the evidence and continuity planning is yours, since you operate the cluster, database, and cloud infrastructure.
...
* *Single region, multiple availability zones*: baseline. Resilient to loss of an AZ. In BYOK8s the zone spread is part of the cluster you operate.
...
[NOTE]
====
This page is part of the BYOK8s Shared Responsibility Model and is updated as Akka's platform and operational practices evolve. To receive notifications of material updates, email mailto:support@akka.io[support@akka.io].
====
