...
Start with a tabletop: walk the failure scenarios end-to-end, confirm who does what, and capture the prerequisites and gaps. From there, choose the scenario(s) that matter most and exercise them live. The xref:operations:production-readiness/byok8s/disaster-recovery.adoc[disaster recovery scenario catalog] is the detailed reference the tabletop walks through.
...
[NOTE]
====
This page is part of the BYOK8s Shared Responsibility Model and is maintained as a living reference, updated periodically as Akka's platform and operational practices evolve. To receive notifications of material updates, email mailto:support@akka.io[support@akka.io].
====
