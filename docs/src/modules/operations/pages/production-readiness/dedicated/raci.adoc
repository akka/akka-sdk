= RACI
:page-summary: The Dedicated RACI matrix listing every operational activity and its Responsible, Accountable, Consulted, and Informed assignments across the Customer and Akka, with descriptions and cadences.
...
| Cloud Account | Billing & Ownership | A | A |  | C | Akka owns and operates the dedicated cloud account and pays the bill. | 
| Cloud Account | Provisioning | A | A |  | C | Akka provisions and owns the dedicated cloud account it operates in for hosting the Platform. | 
...
| Cloud Account | Execute akka-bootstrap | A | A |  | C | Akka executes the Terragrunt bootstrap script to establish programmatic access within the Cloud Account it owns. | 
| Cloud Account | Terraform State Backend | A | A |  | C | Akka owns and secures the Terraform state backend (cloud storage bucket, locking, encryption) used when running akka-bootstrap in the account it operates. | 
| Cloud Account | Akka Permissions via akka-bootstrap | A | A |  | C | Akka creates and maintains its own permissions within the Cloud Account it owns and operates, managing all IAM roles and bindings required for Platform operation. | 
...
| Networking | Private Connectivity | A | A | C |  | Akka operates connectivity within its own VPC, so the Customer does not select a private-connectivity pattern. Where the Customer integrates their own network (for example, a transit gateway), the parties coordinate on the integration. | 
...
| Cloud/Infrastructure Audit Logging | SIEM / Security Monitoring | A | A |  | C | Akka operates SIEM and security monitoring for the dedicated cloud account and notifies the Customer of relevant findings. (In BYOC this sits with the Customer, who owns the account; in Dedicated it becomes Akka's.) | 
...
| PKI | Customer-Provided TLS Certificates | C | C |  | C | Where the Customer brings its own domain, the Customer provides TLS certificates or configures cert-manager, and Akka validates and deploys them. See https://doc.akka.io/operations/tls-certificates.html#_custom_server_certificates for details. | 
...
| Permissions | Infrastructure Access Control and IAM | A | A |  | C | Akka runs akka-bootstrap and owns account-level IAM for the dedicated cloud account where the region is installed. | 
...
| Setup | Maintenance Window | C | C | A | A | The Customer provides a maintenance window, usually 1-4 hours per week. It is used by Akka for any operation that may cause downtime, including database minor updates (applied automatically by the cloud provider) and manual upgrades such as database major patches. | 
