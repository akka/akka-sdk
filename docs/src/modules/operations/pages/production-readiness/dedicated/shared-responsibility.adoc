...
At a glance::
* *Akka*: Provides database point-in-time recovery, Velero-based Kubernetes backups, periodic restore testing, and multi-region failover tooling and runbooks. Default RTO/RPO targets apply; customer-specific targets can be agreed.
...
| Backup restore testing | Akka | Akka performs periodic restore testing against backups; results are summarized on request. *Quarterly.*
