...
Akka Automated Operations is a managed platform. Under the Dedicated model, Akka owns and operates the cloud account that hosts it and runs the platform within that account: the runtime, operators, encryption, region installation, scaling, patching, observability of the platform, backups, and the operational rituals that surround them, together with the cloud account itself, the in-region network, and the security monitoring over that account. You take responsibility for the applications you build with the Akka SDK, the data those applications process, any customer-managed encryption keys or TLS certificates you bring, and the operational partnership.

Dedicated re-owns the cloud-account layer that the customer holds in BYOC, so it leaves you the least to run of the three deployment models (BYOK8s, BYOC, and Dedicated). Akka manages the platform layer and the account beneath it end-to-end; you manage the applications above; a small number of activities are jointly owned, with the lead and consulted parties named explicitly.
...
Because Akka owns and operates the cloud infrastructure, there is no customer-side infrastructure to coordinate on, except any network you choose to integrate with the Akka region (for example, a transit gateway).
...
| Cloud account and billing | Akka | Akka owns and operates the account, pays the bill, and controls the account IAM.
...
Akka owns and operates the cloud-provider account that hosts the Akka region. In Dedicated, Akka provisions the account, pays for it, holds the account-level IAM, and operates everything inside it. The customer does not own or operate the cloud account.

At a glance::
* *Akka*: Provisions, owns, pays for, and operates the dedicated cloud account. Sets quotas, runs akka-bootstrap, owns the Terraform state backend, and holds all account IAM.
...
| Cloud account provisioning | Akka | Akka provisions and owns a dedicated AWS account, GCP project, or Azure subscription to host the platform. A dedicated account simplifies cost tracking and security boundaries.
...
| Execute akka-bootstrap | Akka | Akka runs the Terragrunt utility to establish programmatic access within the cloud account it owns.
| Terraform state backend | Akka | Akka owns and secures the Terraform state backend (cloud storage bucket, locking, encryption) used when running akka-bootstrap.
...
| Billing | Akka | Akka owns and operates the account and pays the bill.
|===
====
...
At a glance::
* *Akka*: Builds and operates the in-region VPC, load balancers, and certificate issuer, along with CIDR allocation, peering, IP allowlists, and DNS. Publishes the connectivity-pattern whitepaper.
...
| Private connectivity | Akka | Akka operates connectivity within its own VPC, so the customer does not select a private-connectivity pattern. Where the customer integrates its own network (for example, a transit gateway), the parties coordinate on the integration.
| Connectivity pattern whitepaper | Akka | Akka publishes implementation details for each supported pattern, which the customer reviews where a network integration is planned.
...
| Private ingress/egress | Akka | Where private connectivity is used, Akka installs the region with an automatically generated load-balancer IP so internal traffic does not traverse the public internet.
...
Akka auto-scales compute and persistence within the region's defined sizing standards. Because Akka owns the cloud account in Dedicated, Akka also pays the cloud bill and reviews the account's charges.
...
| Cloud-native audit logging | Akka | Akka enables cloud-native audit/security services (e.g., CloudTrail, Cloud Audit Logs, equivalents) for the dedicated account, collects and retains the logs, and investigates suspicious activity, sharing relevant findings with the customer. In BYOC this sits with the customer who owns the account; in Dedicated it becomes Akka's.
| SIEM / security monitoring | Akka | Akka operates SIEM and security monitoring for the dedicated cloud account and notifies the customer of relevant findings. In BYOC these sit with the customer who owns the account; in Dedicated they become Akka's.
