...
Day 2 Ops has no multi-region failover; DR is a cold rebuild, and rehearsing it is the single most important exercise at this tier.

There are many valid ways to run it: how data is exported and where it is kept, how the runbook is structured, which scenarios are simulated live, and how RTO and RPO are measured and agreed all depend on your application and topology. Rather than prescribe one path here, Akka works these through with you as part of DR planning. The essential outcomes:

* [ ] DR runbook agreed and rehearsed end-to-end with Akka (declaration, data restore, region rebuild, cutover, verification)
* [ ] A DR tabletop and a live simulation completed, with gaps captured and re-tests scheduled
* [ ] RTO and RPO measured and accepted by the business

[NOTE]
====
RTO and RPO are the Recovery Time Objective and Recovery Point Objective. Targets are defined in your agreement; see the Shared Responsibility Model.
====
