...
What Akka expects of customer applications before going live in *Dedicated*, the deployment model where Akka Automated Operations runs inside a cloud account Akka owns and operates. Akka handles most of the operational surface, including the cloud account itself, so the customer surface is the smallest of the three deployment models. Production is still a shared outcome; the items below are the customer's side of that line.
...
This page is a guideline. The authoritative checklist for your application is the one you build with Akka.

In Dedicated, Akka owns and operates the cloud account. Account, networking, and cloud-security items are Akka's. The items below focus on the applications you build.

Walk this page before going live: use the at-a-glance below as the quick entry point and the detailed checklists for specifics. The items are a baseline drawn from Dedicated deployments; add anything specific to your request flows and failure modes. Discuss modifications or removals with your Forward Deployed Engineer (FDE), the named Akka engineer assigned to your account, so the original rationale is shared and the change is evaluated jointly.

The agreement and the Federation Plane SLA Policy are the authoritative statement of what is committed. Reaching the reliability and availability of your deployment depends on working these items through with your Akka team.

Each item is yours to verify. Akka recommends not advancing to production until each is covered. Engage your FDE and Akka's customer success and support teams throughout the work, or complete it independently.

Items split across two levels. *Region-level* items are established once for the region and shared by every application running on it (disaster recovery, backups, baseline platform alerting). *Application-level* items are handled per service (sizing, application observability, rollback paths). This page lists both; your Akka team's tailored checklist assigns each item to the right level, so region-level work is not repeated per application, though it still recurs on its own cadence.

Akka operates the full stack beneath your applications: the cloud account, the in-region network, account security monitoring, the runtime, region installation, scaling, patching, platform observability, backups, and the operational rituals around them. The items on this page are the customer-side complement. Your FDE walks the principal items before go-live.

This page targets the *Day 2 Ops* engagement (1 dev region + 1 prod region, single prod region with cold-rebuild DR (Disaster Recovery)). The final section covers the additional requirements for the *Business Continuity* tier (multi-region with cross-region failover).

[NOTE]
.*Tier note*
====
If the business requires rapid, automated cross-region failover, Day 2 Ops is not the right tier. DR at Day 2 is a cold rebuild; recovery is on the order of hours, driven by data-export frequency and rebuild time. Confirm with the business early.
====

.*At a glance, what we ask of you*
[NOTE]
====
. *Size with Akka; capture the result in writing.* The FDE runs the sizing exercise once. Cluster shape, broker capacity, and dev/prod parity live in version control alongside your descriptors.
. *Wire your observability.* Logs, metrics, and traces flow to your enterprise platform via the Akka-supplied exporter. Akka cannot see your application data, so observability is yours; it is what lets your team and on-call answer "is the system healthy?".
. *Make CI/CD routine, including rollback.* Image rollback is the most commonly rehearsed path. Descriptor changes, JWT (JSON Web Token) rotations, and partial migrations need rehearsed rollback paths too.
. *Test under stress, not just happy path.* Run load, soak, spike, and chaos tests against a realistically-sized dev region before production traffic.
. *Rehearse recovery.* DR is a two-session exercise: a tabletop covering scenarios, then a live simulation of the scenarios the customer and Akka agree matter most. Business Continuity adds cross-region failover and failback drills.
...
.*Akka's side*
====
Akka runs a parallel internal pre-production validation against the platform-side commitments in the RACI: backups, replication, observability stack, baseline regional alerts, certificate monitoring, cloud-account audit logging, and security monitoring. This page is the customer-side complement; the two pair together.
====

== Architecture and design review

Akka reviews your application's high-level architecture and design to understand the whole system, not isolated snippets, and to guide it toward production. Run this mid-stream, not just before go-live, so findings can still shape the build.
...
* [ ] Per-service xref:operations:services/instance-sizing.adoc[instance types] chosen for each workload (defaults reviewed; pinned where a service needs a specific shape)
...
Run these against the dev region, sized realistically. Without realistic sizing, results are meaningless.
...
Without multi-region failover, DR is a cold rebuild. Rehearsing it is the most important exercise at this tier.

A DR rehearsal is worth as much for what it surfaces as for the recovery itself. Walking a real recovery end-to-end surfaces the gaps that would otherwise appear mid-incident: who owns each step, a firewall rule that must be opened for a rebuilt database, a data-export destination that was assumed rather than verified, or access no one currently holds. Akka works with you on the most prevalent scenarios.

Start with a tabletop: walk failure scenarios end-to-end, confirm who does what, capture prerequisites and gaps. Then exercise the scenarios that matter most live.
...
* [ ] The most important scenarios from the tabletop exercised live, with gaps fed back and re-tests scheduled
* [ ] DR runbook captured from the exercise: declaration criteria, data restore, region rebuild, cutover, verification

[discrete]
==== Rollback testing

Image rollback is commonly rehearsed. Descriptor changes, JWT rotations, and partial data migrations often are not. Incidents extend when those rollback paths are unrehearsed.
...
These fall between teams and are the most commonly skipped. They are baseline at Day 2 Ops; Business Continuity builds on that baseline.
...
Every plausible failure needs a runbook. Confirm runbooks exist for at least:
...
Where you are regulated, these are the items auditors and regulators ask for. Consider each; skip only with a documented reason.
...
Document the continuity posture of each third-party dependency. Operational response runbooks live under Operational readiness.
...
* [ ] Gaps from the rehearsal closed before launch
...
The sections above cover Day 2 Ops with a single prod region. For the Business Continuity tier (multi-region with cross-region failover), the requirements below layer on top; they do not replace those above.

[IMPORTANT]
.*Tier requirement*
====
Akka reads from both regions, so cross-region failover is fast (often sub-minute). Replication lag on recent writes is driven by overall system load, not a tunable platform setting. The bullets below are the application-side responsibilities needed to operate correctly in a multi-region setup.
====

[NOTE]
.*Availability level and replication mode*
====
Choose availability level per service.

* *Single region, multiple availability zones*: baseline. Resilient to loss of an AZ. High availability across zones is the default on Akka-operated infrastructure.
* *Multi-region, replicated*: highest availability. Resilient to loss of an entire region, with geographic failover and low-latency local access (xref:concepts:multi-region.adoc[]).

Within multi-region, the replication mode set per service determines failover behavior. The modes (replicated writes; replicated reads with a request-region or pinned-region primary) and their consistency and switch-over semantics are defined in xref:concepts:multi-region.adoc[].
...
.*Required sign-offs*
Gather the sign-offs your application and organization require: architecture, security (CISO / InfoSec), compliance (if regulated), SRE / platform (runbook tested, on-call ready), FinOps (cost forecast accepted), the product owner (accepting the SLOs and recovery window), and the DR tabletop plus its live simulation (or the cross-region failover drill at Business Continuity).
...
.*The gate*
====
A single named owner (individual or designated body such as a go-live board) is accountable end-to-end and has explicitly approved production readiness. Without one, the checklist becomes assumed readiness in which each party presumes another has covered it.
====

== Day-1 and post-launch readiness

These items carry the launch through its first hours and weeks: success criteria, hypercare coverage, and how early findings feed back into the work.
...
. *Sizing assumptions remain undocumented.* When traffic profiles change or staff transition, capacity decisions have no recorded baseline. Capture inputs, assumptions, and cluster shape in version control alongside descriptors.
. *DR and chaos drills are scheduled and repeatedly deferred.* Deferrals extend planned exercises by months. Set a recurring calendar entry, assign a named owner, and treat skipped drills as a sign-off blocker for the next release.
. *Rollback procedures cover code but not configuration.* Image rollback is commonly rehearsed; descriptor changes, JWT rotations, and partial data migrations often are not, and that is where incidents extend.

The most commonly omitted requirement is a single named owner of the application (individual or designated body), accountable for production readiness end-to-end and authorized to declare readiness or identify remaining gaps.

[NOTE]
====
This page is part of the Dedicated Shared Responsibility Model and is updated as Akka's platform and operational practices evolve. To receive notifications of material updates, email mailto:support@akka.io[support@akka.io].
====
