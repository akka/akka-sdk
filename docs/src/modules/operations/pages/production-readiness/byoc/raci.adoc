...
| Maintenance | Infrastructure patch / Vulnerability management | A | A |  | C | Akka manages infrastructure patching and vulnerability remediation for the Platform, including the JVM, the container base image, the operating system, and the Akka runtime that ship with the customer's deployed services. Vulnerability reports are available from Akka's trust center. | 
| Maintenance | K8s / DB Upgrades | A | A |  | C | The Customer should provide a maintenance window. Akka will notify the Customer prior to any maintenance that causes downtime. | 
...
| Observability | Akka Region Platform Telemetry | A | A |  | C | Akka monitors metrics and logs and will notify the Customer as needed. | 
| Observability | Akka Region Platform Telemetry for the Customer (optional) | C | C | A |  | Customer can optionally choose to set up platform telemetry for the cluster and for the database instance as they own the account. Akka can be consulted for components that they can set up observability | 
