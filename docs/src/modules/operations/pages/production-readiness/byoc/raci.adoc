...
| Setup | Maintenance Window | C | C | A | A | The maintenance window is used by Akka for any operation that may cause downtime, including database minor updates (applied automatically by the cloud provider) and manual upgrades such as database major patches. | 
