...
The at-a-glance summary and the per-area sections below assign an owner to each activity. For the authoritative row-by-row breakdown, with the full Responsible / Accountable / Consulted / Informed split, see the xref:operations:production-readiness/byoc/raci.adoc[RACI]. This Shared Responsibility Model is where that RACI is linked from.
...
Shared responsibility extends to going live. Akka has a set of standards Akka expects of customer applications before flipping prod traffic on: sizing, observability, CI/CD, functional and non-functional testing, DR (disaster recovery) and rollback rehearsal. The xref:operations:production-readiness/index.adoc[production readiness] page captures those standards and the sign-off gate Akka recommends before launch.
...
The customer brings the cloud account at the bottom and owns the applications at the top. Akka manages the platform layer in the middle.

....
+-------------------------------------------------------------------------+
| Customer Applications (Akka SDK services)                Customer owns  |
|   - Source, build, deploy            - App ACLs / IAM (JWTs)            |
|   - Service config & secrets         - App observability / SDK logs    |
+-------------------------------------------------------------------------+
| Akka Platform (managed by Akka)                              Akka owns  |
|   - Akka runtime & operators         - Backups, restore testing        |
|   - Region Kubernetes cluster        - Scaling, patching, releases      |
|   - Persistence store                - Federation Plane integration    |
|   - Container registry               - Platform observability          |
|   - Encryption (at rest, in transit, mTLS)                             |
+-------------------------------------------------------------------------+
| Cloud Account (AWS account / GCP project / Azure sub.)   Customer owns  |
|   - Billing & quotas                 - DNS records, IP allowlists      |
|   - IAM that grants Akka access      - Cloud-native audit logs         |
|   - CIDR allocation, peering         - Customer-managed KMS keys        |
+-------------------------------------------------------------------------+
....
