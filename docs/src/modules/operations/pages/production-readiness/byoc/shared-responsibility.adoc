...
Akka manages the lifecycle of the platform itself: patching, upgrades, runtime/infra releases, and security remediations. Most of this maintenance requires no downtime; where downtime is unavoidable, Akka coordinates it with the Customer in advance and operates within the agreed maintenance window. Runtime and SDK releases are announced by release-notification email.
