...
Start with a tabletop: walk the failure scenarios end-to-end, confirm who does what, and capture the prerequisites and gaps. From there, choose the scenario(s) that matter most and exercise them live. The xref:operations:production-readiness/byoc/disaster-recovery.adoc[disaster recovery scenario catalog] is the detailed reference the tabletop walks through.
