...
What Akka expects of customer applications before going live in BYOC (Bring Your Own Cloud), the deployment model where Akka Automated Operations runs inside the customer's cloud account. Akka handles most of the operational surface and protects the deployment within those bounds. Production is still a shared outcome; the items below are the customer's side of that line.
...
Walk this page before going live: use the at-a-glance below as the quick entry point and the detailed checklists for specifics. The items are a baseline drawn from BYOC deployments; add anything specific to your request flows and failure modes. Discuss modifications or removals with your Forward Deployed Engineer (FDE), the named Akka engineer assigned to your account, so the original rationale is shared and the change is evaluated jointly.
...
Akka Automated Operations runs the platform end-to-end inside your cloud account: runtime, region installation, scaling, patching, platform observability, backups, and the operational rituals around them. The items on this page are the customer-side complement. Your FDE walks the principal items before go-live.
...
.*Akka's side*
====
Akka runs a parallel internal pre-production validation against the platform-side commitments in the RACI: backups, replication, observability stack, baseline regional alerts, certificate monitoring. This page is the customer-side complement; the two pair together.
====
...
Start with a tabletop: walk failure scenarios end-to-end, confirm who does what, capture prerequisites and gaps. Then exercise the scenarios that matter most live. The xref:operations:production-readiness/byoc/disaster-recovery.adoc[disaster recovery scenario catalog] is the detailed reference the tabletop walks through.
...
[NOTE]
====
This page is part of the BYOC Shared Responsibility Model and is updated as Akka's platform and operational practices evolve. To receive notifications of material updates, email mailto:support@akka.io[support@akka.io].
====
