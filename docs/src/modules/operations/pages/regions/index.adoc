...
----
+
In the above example, `gcp-us-east1` is the region that was previously downed. The CLI command is sent to `aws-us-east-2`.
. You can send the same command to all regions, including the previously failed region.
+
[source, command line]
----
