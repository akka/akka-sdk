...
==== Workflows
Workflows handle writes, reads and forwarding of requests in a similar way as Event Sourced Entities, but it keeps the primary in the region where the workflow was created. Akka will route update requests to this region from any endpoint. Workflow actions are only performed by the primary Workflow instance.
