...
Using either the `akka route edit` command or updating the route descriptor, for all traffic to a service:
[source,yaml]
----
host: ecommerce.acme.org
routes:
...
Isolating websocket traffic to a separate route, while allowing regular traffic with a separate route:
[source,yaml]
----
host: ecommerce.acme.org
routes:
