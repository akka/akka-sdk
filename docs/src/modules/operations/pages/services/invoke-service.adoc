...
- By <<exposing-internet,exposing services to the internet>> using routes
- <<_proxy_command,During testing and development>> from the console or using the `akka` proxy command
...
A route declares how traffic to a particular hostname gets routed to your services. You can let Akka generate a hostname for you, or you can provide your own hostname at your own domain. If you want to use your own hostname, you will need to register your domain and configure your Domain Name System (DNS) settings using a third party DNS service.
...
If you have a single service you want to expose to the internet using an Akka generated hostname, you can do so using the `akka service expose` command. This command is provided as a convenience, particularly when you are getting started with Akka:
...
When defining a route with a `prefix`, please note the following:
* **No Path Rewriting:** Akka does not strip or rewrite the prefix before passing the request to your service. The service receives the full URI path exactly as the client sent it. For example, if you route `/serviceA` to a service, a request to `/serviceA/doStuff` arrives at the service as `/serviceA/doStuff`, not `/doStuff`.

* **Literal Matching Only:** The `prefix` is matched as a literal string. Wildcards (e.g., `*` or `#`) are not supported and will be interpreted as literal characters.
====

==== Provisioning a custom hostname

If you want to provision a custom hostname, you first need to register it with your project. Only one project can use a given hostname. Suppose the hostname you want to register is called `ecommerce.acme.org`. To register a hostname, use the `akka project hostname add` command:
...
Assume you want to expose two HTTP services:

. `shopping-cart`, which has an HTTP Endpoint with path `/carts`
. `product-info`, which has an HTTP Endpoint with path `/product`
...
Also assume that you want to name the route `acme-ecommerce`, and that the hostname you want to serve it at is `ecommerce.acme.org`, and that you have already added this hostname to the project following the instructions above.
...
Here you can see the exact error message - your hostname is not resolving. To rectify this, you would need to go to your DNS provider and update the DNS record for the host, and then wait for that change to propagate. Once the problem is fixed, there is nothing you need to do, Akka will periodically recheck the DNS configuration to see if it is updated, typically every 20 minutes. Note that DNS configuration can be cached in DNS servers for up to 24 hours, so it may take that long before Akka can see your changes.
...
For a complete reference for the Akka route descriptor, see the xref:reference:descriptors/route-descriptor.adoc[Akka route descriptor reference].
...
IMPORTANT: WebSocket connections require downgrades the internal communication to HTTP/1.1 which breaks gRPC support and
is more costly for regular HTTP traffic than HTTP/2 which is normally used.
For production loads, WebSocket routes should be defined separately from the routes handling
regular HTTP and gRPC traffic to a service.

==== Securing routes

All routes are served with TLS certificates. In addition, you can also enable client certificate authentication, also known as Mutual TLS (mTLS), and customise the certificates it is provisioned with. Instructions for doing this can be found in xref:operations:tls-certificates.adoc[].

[#_http_basic_authentication]
==== HTTP Basic authentication

It is also possible to enable HTTP Basic authentication on routes. HTTP Basic authentication requires configuring a realm, which is returned in the `WWW-Authenticate` header when authentication fails or no authentication headers are present. The realm name can be anything.
...
- xref:reference:cli/akka-cli/akka_routes.adoc#_see_also[`akka routes` commands]
- xref:reference:cli/akka-cli/akka_services.adoc#_see_also[`akka services` commands]
