...
When running more than one instance they must form an Akka Cluster. This is done automatically in a Kubernetes environment. If that is not an option in your environment you can run it as a single instance by defining the following configuration in your `application.conf`:
