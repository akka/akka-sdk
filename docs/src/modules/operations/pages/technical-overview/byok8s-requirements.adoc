...
The requirements above are cloud-agnostic. A complete BYOK8s setup for one cloud has four parts, in order:

. Meet the <<requirements,infrastructure requirements>> and pass the <<smoke-tests,smoke tests>> above.
. Apply your cloud's specifics in its section below.
. Provision the cloud resources listed on the cloud page (xref:operations:technical-overview/aws.adoc[AWS], xref:operations:technical-overview/azure.adoc[Azure], or xref:operations:technical-overview/gcp.adoc[GCP]).
. Complete the xref:operations:production-readiness/byok8s/checklist.adoc[production-readiness checklist] before going live.

[#aws]
=== AWS (EKS)

Apply these AWS additions to the <<requirements,shared requirements>> above.
...
*Next:* provision the AWS resources on the xref:operations:technical-overview/aws.adoc[AWS page], then complete the xref:operations:production-readiness/byok8s/checklist.adoc[production-readiness checklist].

[#azure]
=== Azure (AKS)

Apply these Azure additions to the <<requirements,shared requirements>> above. Register the resource providers and enable EncryptionAtHost as described on the xref:operations:technical-overview/azure.adoc[Azure page]. Use Azure Database for PostgreSQL flexible server for the database, and the Azure secrets-store CSI driver provider.

*Next:* provision the Azure resources on the xref:operations:technical-overview/azure.adoc[Azure page], then complete the xref:operations:production-readiness/byok8s/checklist.adoc[production-readiness checklist].

[#gcp]
=== GCP (GKE)

Apply these GCP additions to the <<requirements,shared requirements>> above. Use Cloud SQL for PostgreSQL (reachable over private services access), Cloud DNS for the two subdomain zones, and the GCP secrets-store CSI driver provider.

*Next:* provision the GCP resources on the xref:operations:technical-overview/gcp.adoc[GCP page], then complete the xref:operations:production-readiness/byok8s/checklist.adoc[production-readiness checklist].
