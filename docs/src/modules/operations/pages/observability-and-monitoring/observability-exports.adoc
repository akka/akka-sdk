...
Resource attributes describe the service that produced a metric, a log record, or a trace span. Akka sets a standard set of them, such as `service.name` and `akka.region`. You can add attributes of your own, such as the owning team or a support contact, and then filter and group by them in your monitoring backend.

Setting a custom resource attribute takes two steps in the service descriptor. First, set the value as a service label or annotation. Then list its key under `telemetry/resourceAttributes`:
...
xref:operations:services/deploy-service.adoc#apply[Deploy the service with the descriptor]. Akka applies the change with a rolling restart of the service.

Akka adds the attributes to the metrics, logs, and traces of that service, and sends them to every exporter configured for the project. Declare the same key in several services to group their telemetry by that value.

When you apply the descriptor, Akka rejects the configuration if:

* A key is not declared in `labels` or `annotations`, or is declared in both.
* A key starts with `service.`, `telemetry.`, `otel.`, `k8s.`, `process.`, or `akka.`. Akka reserves those prefixes for the attributes it sets itself.

NOTE: Labels and annotations are disabled by default and are enabled per organization. Contact support to enable them.
...
- xref:reference:descriptors/service-descriptor.adoc[]
