...
The JSON log messages include a `trace_id` field when tracing is xref:operations:observability-and-monitoring/observability-exports.adoc#activating_tracing[enabled].
