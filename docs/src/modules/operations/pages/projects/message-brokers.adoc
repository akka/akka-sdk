...
Akka eventing integrates with _Google Cloud Pub/Sub_ and managed Kafka services such as _Confluent Cloud_, _Amazon Managed Streaming for Apache Kafka (Amazon MSK)_, _Aiven for Apache Kafka_, and _Azure Event Hubs_ to enable asynchronous messaging and integrations with other systems.
...
For running Akka services that integrate with a message broker locally, see xref:sdk:running-locally.adoc#_local_broker_support[running a service with broker support].
