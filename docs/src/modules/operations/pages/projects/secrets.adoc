...
----
akka service deploy my-service my-image:latest \
  --secret-env DB_USER=db-secret/username,DB_PASS=db-secret/password,DB_HOST=db-secret/host <1>
----
+
<1> Maps three environment variables to three different keys within the same `db-secret`
--
