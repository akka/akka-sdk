...
Akka provides a built-in _Akka Container Registry (ACR)_ which is pre-configured for your convenience. Alternatively, xref:operations:projects/external-container-registries.adoc[external container registries] are also supported for Akka.
...
See xref:operations:services/deploy-service.adoc[deploy and manage services] for further details on how to deploy a service.

== See also

- xref:reference:cli/akka-cli/akka_container-registry.adoc#_see_also[`akka container-registry` commands]
