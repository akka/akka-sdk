= Running Akka without the full platform

include::ROOT:partial$include.adoc[]

ifdef::todo[TODO: intro]

== License key

You can retrieve a free license key from akka.io[https://account.akka.io/key]. Select the Akka SDK option.
...
You define connection configuration in your `application.conf` or use the predefined environment variables.
...
You need to create and operate the Kubernetes cluster yourself. For example using Amazon EKS.
...
[source,yml]
----
include::java:example$doc-snippets/kubernetes/namespace.yml[]
----
...
[source,yaml]
----
include::java:example$doc-snippets/kubernetes/rbac.yml[]
----
...
[source,yaml]
----
include::java:example$doc-snippets/kubernetes/deployment.yml[]
----
+
...
[source,yaml]
----
include::java:example$doc-snippets/kubernetes/service.yml[]
----
...
As a first test you can create a fort forward with:
...
The standalone image can be run in most container image platforms. When starting the JVM you must define the right runtime configuration with `-Dconfig.resource=runtime-standalone.conf`. Such system properties can also be defined with environment variable `JAVA_TOOL_OPTIONS`.

When running more than one instance they must form an Akka Cluster. This is done automatically in an Kubernetes environment. If that is not an option in your environment you can run it as a single instance by defining the following configuration in your `application.conf`:
