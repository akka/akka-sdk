...
This section presents how you create and manage JWTs with Akka. Using JWTs to secure services is presented in xref:java:auth-with-jwts.adoc[Authentication with JWTs in the Akka SDK].
...
* *Specify Required JWTs for Endpoints*: Define which endpoints require JWTs and specify validation criteria for claims. Refer to the xref:java:auth-with-jwts.adoc[Authentication with JWTs in the Akka SDK] for using endpoint security.

== See also
* xref:java:auth-with-jwts.adoc[Authentication with JWTs in the Akka SDK]
* xref:reference:cli/akka_services_jwts.adoc#_see_also[`akka service jwt` commands]
* xref:reference:cli/akka_secrets.adoc#_see_also[`akka secrets` commands]
