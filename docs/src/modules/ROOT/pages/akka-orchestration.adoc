...
Akka Orchestration guides, moderates, and controls long-running processes so that agents and services reliably reach their goals despite failures, delays, or execution that spans seconds, days, or months. It gives you a durable execution engine for coordinating the steps of a business process, a multi-step agent plan, or a distributed transaction — without writing your own retry loops, state machines, or recovery logic.
...
Each step can define timeouts, retry policies, and recovery strategies. When a later step fails, _compensation handlers_ reverse the effects of earlier completed steps — the saga pattern — so a partially executed workflow can be unwound to a consistent state. This is what makes it safe to orchestrate distributed transactions across systems that have no shared transaction boundary.
