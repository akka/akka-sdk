...
Model context windows are finite, and stuffing them degrades both cost and quality. Akka Memory supports xref:sdk:agents/memory.adoc#_compaction[compaction] to keep the working context focused, summarizing or pruning older material so the agent carries forward what matters without exceeding the window.
...
To read the latest value across many entities, xref:sdk:views.adoc[Views] project entity state into a read model you can query, rather than serving as primary storage.
