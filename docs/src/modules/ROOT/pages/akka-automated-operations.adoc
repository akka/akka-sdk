...
:page-summary: The operational plane that runs Akka services in production — elastic scaling, multi-region replication, HA/DR, rolling updates, and live patching.
:page-persona: operator-platform, operator-sre
:page-related: operations:index.adoc

include::ROOT:partial$include.adoc[]

== Overview

Akka Automated Operations (AAO) is the operational plane that runs your agents and services in production. It automates the day-2 work that normally slows teams down: scaling to demand, replicating state across regions, recovering from failures, rolling out new versions, and keeping the whole system observable — all without taking traffic offline.
...
image::capabilities/one-click-deploys-alt.png[Deploy and operate services across environments with one model,width=620]
...
The platform continuously supervises the services you deploy. It provisions capacity, places instances, replicates data, watches health, and reacts to change automatically. Operators set intent — where services should run, how they should scale, how data should move — and the platform enforces it.
...
image::capabilities/muilt-region-replicated-write.png[State replicated across regions with region-aware routing,width=620]
...
This recovery is automatic and does not depend on operator intervention. Failure handling is part of the runtime rather than a manual runbook, so transient faults are absorbed instead of turning into incidents.
...
This holds even when the update changes your data model. AAO coordinates schema and state changes as part of the rollout, so evolving how your agents and services store data does not require downtime or a maintenance window.

image::capabilities/zero-downtime.png[Rolling updates in waves with no downtime,width=620]

== Live patching

Operational changes to a running system — updating configuration, rotating credentials, migrating between environments — are applied while the system stays live.

Certificate and key rotation is automated, so security material is refreshed on an ongoing basis without manual coordination or service interruption. Larger moves are handled the same way: infrastructure upgrades, cloud-to-cloud migrations, federating a serverless deployment into your own VPC, and repatriating workloads back on-premises are all performed through replication with automated cutover, so the running system transitions without going offline.
...
Services can be operated as multi-tenant systems, with access governed across multiple organizations. AAO manages the operational controls that keep tenants isolated and secure — including automated rotation of certificates and keys and oversight of how state is persisted — so a shared platform can serve many tenants without compromising separation between them.
