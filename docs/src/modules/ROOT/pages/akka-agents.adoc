...
* *A model* — the language model the agent reasons with, chosen declaratively and swappable without rewriting the agent.
...
* *A2A (Agent-to-Agent)* — communicate with agents built on other platforms using a common agent-to-agent protocol.
* *ACP (Agent Communication Protocol)* — exchange messages with agents across organizational and platform boundaries.
