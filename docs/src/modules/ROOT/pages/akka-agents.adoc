...
Reach for an autonomous agent when the work is open-ended: the model decides each step from what the last one revealed. Reach for it too when several agents must coordinate, or when a typed result needs its own durable lifecycle. Reach for a request-based agent, paired with a xref:ROOT:akka-orchestration.adoc[workflow] when you need fixed step ordering, for request-response interactions. See xref:sdk:autonomous-agents.adoc[Autonomous Agents] for the full component reference.
