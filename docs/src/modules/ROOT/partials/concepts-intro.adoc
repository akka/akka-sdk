...
You implement orchestration by creating an Akka service with the xref:sdk:workflows.adoc[Workflow] component.
...
You implement an agent by creating an Akka service with the xref:sdk:agents.adoc[Agent] component.

You implement a tool in a regular Java class or embedded within the xref:sdk:agents.adoc[Agent] component.

You implement an MCP server with the xref:sdk:mcp-endpoints.adoc[MCP Endpoint] component.

You implement APIs that can front an agent with the xref:sdk:http-endpoints.adoc[HTTP Endpoint] and xref:sdk:grpc-endpoints.adoc[gRPC Endpoint] components.
...
Short-term (traced and episodic) memory is included transparently within the xref:sdk:agents.adoc[Agent] component.

You implement long-term memory with the xref:sdk:event-sourced-entities.adoc[Event Sourced Entity] and xref:sdk:key-value-entities.adoc[Key Value Entity] components.

You implement propagations of cross-system state with the xref:sdk:views.adoc[View] component. Views implement the Command Query Responsibility Segregation (CQRS) pattern.
...
You produce events to a message broker with the xref:sdk:consuming-producing.adoc#_event_producer[Producer] annotation.

You create a continuous incoming stream of events with the xref:sdk:http-endpoints.adoc[HTTP Endpoint] or the xref:sdk:grpc-endpoints.adoc[gRPC Endpoint] components.

You create a stream processor to analyze and act against a stream of data with the xref:sdk:consuming-producing.adoc[Consumer] component.
|===
