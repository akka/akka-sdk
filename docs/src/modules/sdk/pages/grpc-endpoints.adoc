...
== External protobuf message types

The set of common message types defined in the protobuf package `google.protobuf`, for example `google.protobuf.Timestamp`
and `google.protobuf.google.protobuf`, are always available for use in projects with gRPC endpoints.
...
**Handling such protobuf files can be in different ways:**

Copy the needed messages and service descriptors into the project and keep them in version control. gRPC and protobuf
...
and unpack it so that the message types and gRPC service definitions are available. This requires a bit of extra configuration
in the maven project `pom.xml`:

[source,xml,indent=0]
.{sample-base-url}/doc-snippets/pom.xml[pom.xml]
----
include::example$doc-snippets/pom.xml[tag=third-party-proto-jar]
----
