...
* link:{sample-base-url}/changelog-agent[release-note-summarizer,window="new"] -- agent that autonomously summarizes release notes from upstream sources
* link:{sample-base-url}/medical-tagging-agent[medical-discharge-tagging,window="new"] -- agent that classifies and tags medical discharge records
* link:{sample-base-url}/temperature-monitoring-agent[temperature-monitoring,window="new"] -- agent that monitors IoT sensor data and triggers alerts
