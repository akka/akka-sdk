...
In Enforced mode an organization authors a floor of mandatory exit conditions and definitions of done -- security, compliance, repository hygiene, its "way of working" -- that Akka enforces and developers can tighten but never loosen. The floor is versioned policy-as-code in the organization's own git repository, and each condition is assigned a governance level (`always-apply`, `never-apply`, `on-but-dev-configurable`, `off-but-dev-configurable`). The organization's own CI is the enforcement backstop. See xref:reference:governance/policy-as-code.adoc[Policy as code] for the format and the governance levels.
