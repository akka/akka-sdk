...
| *Delegated*
| A condition with `autonomy: attested`, satisfied by a recorded xref:reference:specify/attestations.adoc[attestation]. It resolves DEFERRED until the attestation exists, then GREEN.
|===
...
| *GREEN* | The auditor ran and the invariant holds.
| *RED* | The auditor ran and the invariant is violated -- or a prohibition condition has no auditor, which reds rather than passing silently.
| *OPEN* | Active but not yet resolved: not yet evaluated, awaiting a human sign-off, or a check whose command you have not yet approved.
| *DEFERRED* | Not verifiable locally -- a delegated (attested) check before its attestation, or a requirement unavailable here (for example, a live-service check before deployment). Carried downstream and recorded in the conformance receipt.
| *WAIVED* | Unmet, but covered by a time-bound waiver.
