...
. *Ship*. `/akka:ship <review|release>` runs the auditors for every applicable exit condition in scope for the target and, on pass, runs the ship steps the organization declared for that target.
...
Author-time is a precondition, not a ship target. Every ship — review or release — refuses if any applicable author-tier condition is not green, before running any higher-tier auditors. See xref:reference:specify/exit-condition-states.adoc#ship-tiers[Ship tiers].
