...
.{sample-base-url}/event-sourced-customer-registry/src/main/java/customer/Bootstrap.java[Bootstrap.java]
----
include::example$event-sourced-customer-registry/src/main/java/customer/Bootstrap.java[tag=object-mapper]
----
...
It’s *highly recommended* to add a `@TypeName` annotation to sealed interfaces, such as events. Information about the type, persisted together with the JSON payload, is used to deserialize the payload and to route it to an appropriate `Consumer` or `View` handler. By default, a FQCN is used, which requires extra attention in case of renaming or repacking. Therefore, we recommend using a logical type name to simplify refactoring tasks. Migration from the old name is also possible, see xref:serialization.adoc#_renaming_class[renaming class].
...
Since there is no way to mark a sealed interface for the distinct event types
`applyEvent` must accept `com.google.protobuf.GeneratedMessageV3` and do its own type
matching for the expected message types.

The concrete Event Sourced Entity class must also have the annotation
...
the concrete message types are inferred. For all other cases the consumer class must be annotated with
...
the concrete message types are inferred. For all other cases the updater class must be annotated with
`akka.javasdk.annotations.ProtoEventTypes` listing all event types that the updater will accept.
