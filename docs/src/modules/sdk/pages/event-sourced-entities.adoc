...
NOTE: The above uses the Java `record` to reduce boilerplate code, but regular classes work too as long as they can be serialized to JSON (e.g. using Jackson annotations).
...
Calling a command handler through the TestKit returns a link:{attachmentsdir}/testkit/akka/javasdk/testkit/EventSourcedResult.html[`EventSourcedResult`, window="new"]. This class has methods to assert the handling of the command, such as:
