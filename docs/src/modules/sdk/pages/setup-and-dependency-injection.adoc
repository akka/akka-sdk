...
Components defined in a library jar on the classpath can be added to the service through configuration. List the fully qualified class names under the key for the component type they implement in `application.conf`:

[source,hocon]
----
akka.javasdk.library.components {
  http-endpoint = ["com.example.library.OrderEndpoint", "com.example.library.CustomerEndpoint"]
}
----

The listed classes are loaded on startup as if they were defined in the service itself. The service logs the added library components at startup.

The config block has one key per component type:

[options="header" cols="1,1"]
|===
|Component type |Config key

|Event Sourced Entity |`event-sourced-entity`
|Key Value Entity |`key-value-entity`
|Workflow |`workflow`
|View |`view`
|Consumer |`consumer`
