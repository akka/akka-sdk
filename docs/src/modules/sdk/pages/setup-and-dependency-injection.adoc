...
The callback runs on the SDK dispatcher. It must not throw — any exception it throws is logged and swallowed. It is invoked once per caught exception on the instance that handled the failing request, so in a service running multiple distributed instances each instance reports its own failures.
...
WARNING: Don't use `ConfigFactory.load()` since that will not load the `application.conf` as you intended. Use dependency injection of `Config` instead.
