...
See xref:consuming-producing.adoc#consumer-error-handling[Consumer error handling] for what happens when processing fails.
...
.{sample-base-url}/event-sourced-counter-brokers/src/main/java/counter/application/CounterEventsConsumer.java[CounterEventsConsumer.java]
----
include::example$event-sourced-counter-brokers/src/main/java/counter/application/CounterEventsConsumer.java[tag=ese-consumer-from-snapshot]
----
...
The event handler in the producer should depend only on the incoming event and have no side effects. Derive the public event from the incoming event, or ignore it. Do not call other components, endpoints, or external systems from the handler. The consuming service replays the producer stream when it starts a new subscription or falls behind, so the handler runs again for events that were already transformed. The handler may also be invoked in parallel for different events, and there is no ordering guarantee between the events it receives. Do not keep state in the handler or depend on the order of invocations. A handler without side effects keeps replays fast and gives every consumer the same public events for the same source events. If a consumer needs data from other components, it looks that data up on the consuming side.

xref:consuming-producing.adoc#_starting_from_snapshot[] can be used for service-to-service eventing. The snapshot handling is defined on the producer side using `@Produce.ServiceStream`, where the snapshot is transformed to an event that consumers receive like any other event.
...
IMPORTANT: Deduplication is not handled automatically by Akka. You must implement deduplication logic explicitly in your consumer code.

For detailed guidance on implementing deduplication strategies, including idempotent updates and sequence number tracking, see xref:dev-best-practices.adoc#message-deduplication[Message deduplication].
...
image::eventing-testkit-sample.svg[]
