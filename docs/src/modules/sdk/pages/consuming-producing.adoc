...
The event handler in the producer should be a pure transformation. Derive the public event only from the incoming event, or ignore it. Do not call other components, endpoints, or external systems from the handler. The consuming service replays the producer stream when it starts a new subscription or falls behind, so the handler runs again for events that were already transformed. A pure handler keeps replays fast and gives every consumer the same public events for the same source events. If a consumer needs data from other components, it looks that data up on the consuming side.
