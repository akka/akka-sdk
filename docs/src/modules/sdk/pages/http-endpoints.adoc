...
Note that browsers only use HTTP GET requests for SSE. Other HTTP methods will not be possible to consume through
...
Browsers implementing SSE has reconnecting built in, but it important that SSE endpoint methods are designed with restarts in mind and does not rely on the stream itself to keep a local JVM object alive. A reconnect may not end up in the same service instance as the original connection.
...
Note that on client reconnects, this example will start the reconnected stream issuing the full query, and then start streaming individual updates.
