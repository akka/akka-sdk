...
These methods accept a `Flow` that defines how messages flow between the client and server.

The `Flow` can be a request-response type of interaction where each incoming message from a client leads to one or more response message.
Another alternative is separate, detached, input and output streams using `Flow.fromSinkAndSource`.

Here's an example that streams view updates over WebSocket:
