...
:page-component-type: endpoint
:page-summary: An HTTP Endpoint exposes a service over HTTP, serving JSON APIs, server-sent event and WebSocket streams, and the files of a web application.
:page-when-to-use: REST APIs, browser-facing services, streaming responses, hosting web application assets
:page-related: sdk:web-applications.adoc, sdk:grpc-endpoints.adoc, sdk:access-control.adoc
:page-persona: builder-developer

include::ROOT:partial$include.adoc[]

image:ROOT:endpoint.png[Endpoint,width=100,float=left] An Endpoint is a component that creates an externally accessible API. Endpoints are how you expose your services to the outside world. Two different types of endpoints are available: HTTP endpoints and gRPC endpoints. This page covers HTTP endpoints.
...
=== Serving web application assets ===
...
This section covers the endpoint API. For serving a single-page application, using your own domain name, pushing updates to an open page, and the boundaries of what the service delivers, see xref:sdk:web-applications.adoc[Web applications].
