...
IMPORTANT: While WebSocket endpoint methods work out of the box in local development and tests it requires additional setup to work in a deployed service, see xref:operations:services/invoke-service.adoc#websockets[Invoking Akka Services / Websockets]. If this is not setup WebSocket connections to the deployed service will fail with an HTTP 403 Forbidden response.
