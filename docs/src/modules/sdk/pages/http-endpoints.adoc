...
When the request path ends with `/` or is exactly the prefix with no trailing slash (e.g. `GET /static` or `GET /static/`), `index.html` from that directory is served automatically. This makes it straightforward to serve single-page applications where the root of the static subtree should return the application shell.
