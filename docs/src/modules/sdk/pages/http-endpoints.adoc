...
Streaming responses from an HTTP endpoint is tied to the specific instance they where connected to when the request was first made.
...
on the use case it might be possible to continue just emitting changes by extracting event ids and handle a `RequestContext.lastSeenSseEventId`
...
link:https://datatracker.ietf.org/doc/html/rfc6455[WebSocket] provides full-duplex communication channels over a single TCP connection. Unlike server-sent events which are unidirectional (server to client), WebSocket allows bidirectional streaming between the client and server.
...
ifdef::todo[TODO: details/link about this not working in a deployed service unless special routes are created]

==== Creating a WebSocket endpoint

A WebSocket endpoint is created by returning an `HttpResponse` from `HttpResponses.textWebsocket` or `HttpResponses.binaryWebsocket`.
These methods accept a `Flow` which will be fed incoming messages from the client, and send messages coming out of the flow back to the client.
...
.{sample-base-url}/key-value-customer-registry/src/main/java/customer/api/CustomerEndpoint.java[CustomerEndpoint.java]
----
include::example$key-value-customer-registry/src/main/java/customer/api/CustomerEndpoint.java[tag=ws-view-updates]
----
<1> Query the view to get a stream of customer updates
<2> Convert each customer object to a JSON string using `JsonSupport.encodeToString`
<3> Create a text WebSocket response using `HttpResponses.textWebsocket`
<4> Pass the `RequestContext` which is needed for WebSocket protocol handling
<5> Create a `Flow` that ignores incoming client messages and streams view updates to the client

For binary WebSocket support, use `HttpResponses.binaryWebsocket` which accepts a `Flow` with `ByteString` instead of `String`.
...
[source,java,indent=0]
.{sample-base-url}/helloworld-agent/src/main/java/com/example/api/HelloWorldWebSocketEndpoint.java[HelloWorldWebSocketEndpoint.java]
----
include::example$helloworld-agent/src/main/java/com/example/api/HelloWorldWebSocketEndpoint.java[tag=class]
----
<1> Response is created using a WebSocket stream of strings.
<2> `Flow.flatMapConcat` turns each incoming request into a stream of response strings

`flatMapConcat` means that each response will be streamed until completion, before the next response starts.

==== WebSocket protocol negotiation

WebSocket supports protocols that can be negotiated, the client sends zero to many protocol names that it supports. The server
endpoint then chooses one of the supported protocols and returns an appropriate `Flow` together with the chosen protocol name
in a `SelectedWebSocketProtocol`.

There is one `HttpResponses.binaryWebsocket` and one `HttpResponses.textWebsocket` method accepting such a protocol selector.
