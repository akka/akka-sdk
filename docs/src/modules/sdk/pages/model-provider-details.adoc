...
Structured judgment requests use a separate provider, configured under `akka.javasdk.agent.judgment-model-provider`, see <<SystemOne>> and xref:sdk:agents/judgment.adoc[Structured judgment].
...
A SystemOne model answers structured judgment requests, see xref:sdk:agents/judgment.adoc[Structured judgment]. It is configured under `akka.javasdk.agent.judgment-model-provider`, not under `model-provider`, because it is not a chat model.
...
| provider
| "system-one"
| Name of the provider. Must always be `system-one`

| api-key
| String
| The API key, sent as a bearer token. Defaults to the value of the `SYSTEM_ONE_API_KEY` environment variable

| model-name
| String
| The name of the model to use. Defaults to `jev-latest`, the most recent stable release of the TypeSafe Jev API

| base-url
| Url
| The base URL of the SystemOne API. Defaults to the TypeSafe Jev API. The request goes to `/v1/systemone` under it
...
| max-retries
| Integer
| Retry this many times when the API responds with rate limited (429) or overloaded (529)

| additional-model-request-headers
| List of String
| List of `"name:value"` strings for HTTP headers to include in every request to the API. Inherits from `akka.javasdk.agent.additional-model-request-headers` by default. Can also be set via `ADDITIONAL_SYSTEM_ONE_REQUEST_HEADERS_N` environment variables.

|===

See link:{attachmentsdir}/api/akka/javasdk/agent/JudgmentModelProvider.SystemOne.html[`JudgmentModelProvider.SystemOne`] for programmatic settings.
...
[source,hocon,indent=0]
----
include::example$akka-javasdk/src/main/resources/reference.conf[tag=system-one]
----
