...
| List<String>
| List of `"name:value"` strings for HTTP headers to include in every request to the model API. Inherits from `akka.javasdk.agent.additional-model-request-headers` by default. A single comma-separated string is also accepted (e.g. when set via environment variable).
...
[source,hocon,indent=0]
----
akka.javasdk.agent.openai.additional-model-request-headers = ["Authorization:Bearer openai-token"]
----

=== Environment variable

The global setting can also be supplied via the `AKKA_AGENT_ADDITIONAL_MODEL_REQUEST_HEADERS` environment variable as a comma-separated string, which is the recommended approach for secrets such as gateway tokens:

[source,bash,indent=0]
----
AKKA_AGENT_ADDITIONAL_MODEL_REQUEST_HEADERS="Authorization:Bearer mytoken,X-Custom-Header:value"
----
