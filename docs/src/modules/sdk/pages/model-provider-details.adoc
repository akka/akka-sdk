...
All built in model providers except Local AI support attaching extra HTTP headers to every request sent to the model API. This is useful for passing authentication tokens to an AI gateway or proxy, adding routing or tracing headers, or meeting any other header requirements imposed by the endpoint.

Local AI drops the headers. Use the OpenAI provider with a base URL to reach an OpenAI-compatible server with headers.

A custom provider builds its own chat model. Override `withAdditionalModelRequestHeaders` to take the headers and add them where the chat model is created.
...
Set headers on a model provider built in code. They replace any configuration value for that provider:
...
Set headers on `ModelProvider.fromConfig` when the value is only known at request time, such as a token
the agent reads from its own context. The provider still comes from configuration, so you do not repeat
the model name or the API key:

[source,java,indent=0]
----
ModelProvider.fromConfig()
    .withAdditionalModelRequestHeaders(
        List.of(RawHeader.create("X-Tenant", tenantId)));
----

Headers set on `fromConfig` are added to the headers of the provider that the configuration resolves to,
so a gateway token set in configuration survives. A header name set in both keeps the value from code, and
the configured one is dropped. Header names are compared ignoring case.

Use the provider on one interaction with `effects().model(...)`, or on a whole autonomous agent with
`modelProvider(...)` on its definition. An autonomous agent reads its definition once at startup, so the
headers are the same for every instance of that agent.

== Identity headers

The runtime adds two headers to each model request, so that a gateway or proxy in front of the model API
...
| x-akka-agent-id | The component id of the agent that made the request
| x-akka-flow-id | The flow the request belongs to, or the session id when the request has no flow

|===

Turn them off for every provider:

[source,hocon,indent=0]
----
akka.javasdk.agent.identity-headers = off
----

Each provider section inherits the global value and can override it:
...
A provider named in configuration takes the value from the section it resolves to. A provider built in code
takes the global value, because it has no section.

Local AI does not send these headers. A custom provider builds its own chat model, so the runtime does not
add these headers to its requests.
