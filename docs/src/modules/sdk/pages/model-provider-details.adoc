...
Most built in model providers support attaching extra HTTP headers to every request sent to the model API. This is useful for passing authentication tokens to an AI gateway or proxy, adding routing or tracing headers, or meeting any other header requirements imposed by the endpoint.

Bedrock and Local AI drop the headers, and so does a custom provider, because those providers build their client themselves. A custom provider can add headers where it creates the chat model.
...
The global setting can also be supplied with an environment variable:

[source,bash,indent=0]
----
MODEL_REQUEST_IDENTITY_HEADERS=off
----
...
Bedrock, Local AI and custom providers never send these headers, for the same reason they drop the extra
headers above.
