...
[source,hocon,indent=0]
----
include::example$akka-javasdk/src/main/resources/reference.conf[tag=agent-custom-model-config]
----

The `provider` value must be a fully qualified class name (e.g. `com.example.application.MyModelProvider`). Any additional properties in the configuration section (such as `model-name` or `api-key`) are available to the provider implementation through an injected `Config` parameter.
