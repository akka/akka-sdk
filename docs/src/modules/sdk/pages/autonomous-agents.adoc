...
NOTE: An Autonomous Agent is a heavier process than a request-based Agent. Each instance carries durable orchestration state, a task queue, and lifecycle bookkeeping. For simple request-response interactions a request-based Agent is the lighter and clearer choice. However, a request-based agent must often be combined with a Workflow for orchestration, and then the choice isn't about weight.
