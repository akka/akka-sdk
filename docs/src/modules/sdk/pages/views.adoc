...
TIP: You should be able to test if a change is compatible locally by running the service with xref:sdk:running-locally.adoc#persistence-enabled[persistence mode enabled], producing some data, and then changing the View query and re-running the service. If the service boots up correctly and is able to serve the new query, the change is compatible.
