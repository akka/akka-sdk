...
The Akka SDK testkit has built-in support for simulating message brokers. See xref:sdk:consuming-producing.adoc#testing[Testing the Integration] for more details. For running locally with a broker, refer to xref:sdk:running-locally.adoc#_local_broker_support[running a service with broker support].
