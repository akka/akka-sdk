...
IMPORTANT: When you manage tasks and agents separately, you are responsible for stopping the agent. Autonomous agents do not auto-passivate when their queue drains. An agent that is not stopped holds its in-memory state until you call `stop()`, even though it is idle and making no model calls. The `runSingleTask` shortcut handles this automatically. The explicit `assignTasks` flow does not. Pause is another way to release the actor: a paused agent passivates after a short delay, and resume rehydrates it.
...
A running agent can be paused and later resumed. While paused, the agent stops iterating: it will not start new model calls or process queued tasks. Tasks already assigned remain in the queue and are processed when the agent resumes.
...
The same shape applies to `pauseAsync()`, `resumeAsync()`, `stopAsync()`, `assignTasksAsync(...)`, `runSingleTaskAsync(...)`, and the `forTask` operations. Each returns a `CompletionStage` that completes once the runtime acknowledges the operation. Mixing styles is fine: use synchronous calls where they read more clearly, async ones where the call needs to compose with other futures.
