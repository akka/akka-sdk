...
----
include::example$autonomous-agent-playground/src/main/java/demo/devteam/application/DeveloperTasks.java[]
----
...
[source,java,indent=0]
----
var task = DeveloperTasks.IMPLEMENT.params(Map.of(
  "feature", "rate limiter",
  "requirements", "10 requests per second per user, with 1-minute window"));
----
...
A task reaches a terminal state in one of three ways. The model decides the work is done and produces a result that conforms to the task's declared result schema, and the task transitions to `COMPLETED`. The model decides it cannot make progress and reports a reason, and the task transitions to `FAILED`. Or the iteration limit is reached without either decision, in which case the runtime terminates the task as `CANCELLED`. The status field on the task entity records which path was taken.
...
The notification stream distinguishes the two failure modes. An agent's explicit decision to fail emits `Notification.TaskFailed`. Framework termination, for example after max iterations or a dependency failure, emits `Notification.TaskCancelled`. The task snapshot reports the resulting status. See xref:sdk:autonomous-agents/notifications.adoc[Notifications] for the full event catalog.
...
var status = snapshot.status();        // PENDING, ASSIGNED, IN_PROGRESS, RESULT_REJECTED,
                                       // COMPLETED, FAILED, or CANCELLED
var result = snapshot.result();        // Optional<ReviewResult>, present if completed
var reason = snapshot.failureReason(); // Optional<String>, present if failed
----
