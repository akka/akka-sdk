...
* *Dependents* waiting on the failed task transition to `CANCELLED` automatically.
...
var status = snapshot.status();        // PENDING, ASSIGNED, IN_PROGRESS, RESULT_REJECTED, COMPLETED, FAILED, or CANCELLED
...
// TODO: how to subscribe to the task notification stream from user code. The current `componentClient.forTask(...)` does not expose a stream; access goes through `TaskEntity` directly. This API will be revisited.
