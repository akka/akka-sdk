...
Handoff:: The source agent writes a free-form summary into the handoff tool's `context` argument. That summary, along with any from earlier handoffs, is appended into the next agent's context. There is no automatic summarization of session history.
