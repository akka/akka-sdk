...
TIP: If you are unable to add the marketplace, clone the Akka plugin repository and add it as a local marketplace instead: `git clone https://github.com/akka/ai-marketplace.git` then `/plugin marketplace add /path/to/ai-marketplace`
...
[TIP.manual]
====
If you are unable to install the marketplace plugin or clone the repository, fall back to the Akka CLI:

. Install the Akka CLI (see xref:operations:cli/installation.adoc[]).
. Run `akka specify init <dir>` to provision a new AI-assist ready project.
. Launch your AI coding assistant from the new project directory.

The `_mcp.json` file included in the project template lets you manually register Akka as an MCP server for any compatible agent.
====
...
== Initializing a spec-driven project
To start a new spec-driven project, use the `akka specify init` command instead of `akka code init`. It provisions everything you need for the spec-driven development cycle.

If you installed the <<_step_1_install_the_akka_plugin,Akka Specify Plugin>> (recommended), the `/akka:setup` skill handles project initialization for you. The information below describes what happens under the hood, and applies if you used the `akka specify init <dir>` fallback.
...
You do not have to leave your agent chat during the spec-driven SDLC. The `/akka.build` command does more than `mvn compile exec:java`:
...
== Summary
The full spec-driven cycle is `init` -> `specify` -> `clarify` -> `plan` -> `tasks` -> `implement` -> `build` -> `inspect` -> `deploy`. Each step produces a version-controlled artifact, and the AI assistant generates the code from those artifacts. Scope features small enough that the acceptance criteria are clear and the generated code is easy to review.
