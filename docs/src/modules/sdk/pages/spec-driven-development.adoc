...
Before you start, xref:getting-started:set-up-dev-env.adoc[set up your dev env] -- install the Akka Specify plugin in your AI coding assistant and run `/akka:setup` to configure the CLI, Java, Maven, and your Akka download token. The rest of this page assumes your environment is ready.
...
The short description of the feature is turned into a branch name like `00#-feature-short-description` where the description you supply is converted to _kebab case_. This also becomes the name of the new git branch.

The prompt for the feature specification should exclusively define the _what_ and _why_ of the feature, and should not contain any technical implementation details. In the sample prompt below for a chess application core website, the prompt mentions that the app does not use federated authentication. While this seems like a _how_ level (plan) declaration, it belongs at the higher specification level because the concept of federation is a high level specification -- the prompt does not mention _how_ federation should be coded.
...
Your AI assistant then analyzes your constitution, your feature specification, and any conversation history you have built up and identifies gaps. It looks for places where it would have to make a decision _and_ it does not already have enough information to make that decision. If you run `akka.clarify` on the chess core users feature, you will likely be asked several questions.
...
It is worth remembering that, like the specify step, the input to the plan step is a _prompt_, not the full and final plan. Your agent produces the final plan as output.
...
In other words: _make sure you agree with the functional requirements before continuing on to the next step_. If you want to add requirements to the plan, do so interactively with your agent and it will update the plan accordingly.
...
* Your service is _exercised through the real endpoints_ to verify functionality. This is like building and exercising a custom Postman script or writing and using your own shell script, except it is easier and automated.
...
You no longer have to invest massive amounts of time in "what-if" scenarios -- you can just _specify and go_. All you need is `init` -> `specify` -> `clarify` -> `plan` -> `tasks` -> `implement` -> `build` -> `inspect` -> `deploy`.
...
[cols="1,~"]
|====
|xref:reference:specify/setup.adoc[`/akka:setup`]               | Configure your environment (CLI, Java, Maven, Akka tokens)
|xref:reference:specify/constitution.adoc[`/akka:constitution`] | Create or update the project constitution
|xref:reference:specify/specify.adoc[`/akka:specify`]           | Supply a prompt to produce your feature spec
|xref:reference:specify/clarify.adoc[`/akka:clarify`]           | Find gaps in your spec
|xref:reference:specify/plan.adoc[`/akka:plan`]                 | Convert your spec into a technical implementation plan
|xref:reference:specify/tasks.adoc[`/akka:tasks`]               | Itemize the work required to build according to the spec
|xref:reference:specify/analyze.adoc[`/akka:analyze`]           | Cross-artifact consistency and quality analysis
|xref:reference:specify/checklist.adoc[`/akka:checklist`]       | Generate a custom checklist for the current feature
|xref:reference:specify/implement.adoc[`/akka:implement`]       | Generate the required code, tests, harnesses, etc.
|xref:reference:specify/review.adoc[`/akka:review`]             | Review code against spec, plan, and constitution
|xref:reference:specify/build.adoc[`/akka:build`]               | Build, test, and run locally with hot reloading
|xref:reference:specify/inspect.adoc[`/akka:inspect`]           | Inspect a running service's runtime state against the spec
|xref:reference:specify/deploy.adoc[`/akka:deploy`]             | Deploy to Akka Automated Operations
|xref:reference:specify/issues.adoc[`/akka:issues`]             | Convert tasks into GitHub issues
|xref:reference:specify/converge.adoc[`/akka:converge`]         | Assess the codebase against spec, plan, and tasks; append remaining work to `tasks.md`
|xref:reference:specify/reliability.adoc[`/akka:reliability`]   | Add or remove resilience-testing instrumentation for the reliability dashboard
|xref:reference:specify/docs.adoc[`/akka:docs`]                 | Generate rendered project documentation into `docs/`
|====

These commands are also listed in the xref:reference:specify/index.adoc[].
...
* xref:reference:specify/auditors.adoc[] and xref:reference:specify/auditor-kinds.adoc[] -- the auditor concept and its three kinds (introspective, provisioned, delegated), together with the xref:reference:specify/coverage-gate.adoc[coverage gate], the xref:reference:specify/adequacy-review.adoc[adequacy review], and xref:reference:specify/requirement-analysis.adoc[requirement analysis]. Feedback that changes a check re-runs the adequacy review, so what is verified stays aligned with your current intent.
* xref:reference:specify/conform.adoc[] -- run the auditors and print the definition-of-done manifest with a verdict (`akka specify conform`, or `akka_ec_conform` from an MCP client).
* xref:reference:specify/harnesses.adoc[] -- provisioned harness assets recorded in `.akka/harnesses.lock`.
