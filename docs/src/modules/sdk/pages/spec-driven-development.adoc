...
=== Install the Akka plugin

The fastest way to get started is to install the Akka plugin from the AI Marketplace. In Claude Code, run these commands:

. **Add the Akka marketplace:**
+
[source]
----
...
----
+
This registers the Akka plugin catalog with Claude Code.
...
----
+
This installs the Akka skill, which includes the CLI, MCP server, AI coding assistant, and project scaffolding.
...
----
+
This activates the installed plugin and makes all SDD skills available.
...
=== Alternative: Install from a cloned repository

If the AI Marketplace is not available in your environment, you can clone the Akka plugin repository and install it locally:

. **Clone the Akka AI marketplace repository:**
+
[source,bash]
----
...
. **In Claude Code, add the cloned directory as a local marketplace:**
+
[source]
----
...
. **Install the Akka plugin from the local marketplace:**
+
[source]
----
/plugin install akka-specify@ai-marketplace
...
NOTE: Not using Claude Code? Akka supports 30 different AI assist tools. Visit https://doc.akka.io[doc.akka.io] for installation instructions for your tool.
...
If you installed the Akka plugin (recommended), the `/akka-specify:setup` skill handles project initialization for you. The information below describes what happens under the hood, and applies if you used the `akka specify init <dir>` fallback.
...
* {url-blog-demo-code-init}[Akka Code Init Demo, window="new"] -- see an Akka project scaffolded live.
