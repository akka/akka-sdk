...
* xref:reference:specify/auditors.adoc[] -- the declarative delegation model, where each exit condition delegates its check to the ecosystem's own toolchain, together with the xref:reference:specify/coverage-gate.adoc[coverage gate], the xref:reference:specify/adequacy-review.adoc[adequacy review], and xref:reference:specify/requirement-analysis.adoc[requirement analysis]. Feedback that changes a check re-runs the adequacy review, so what is verified stays aligned with your current intent.
