...
:page-related: xref:ai-coding-assistant.adoc[], xref:operations:cli/installation.adoc[]
...
*What it does.* SDD provides a structured, iterative workflow -- specify, clarify, plan, task, implement, build, deploy, inspect -- backed by a constitution of unbreakable project rules. Every artifact is version-controlled, and automated CI can detect divergence between specification and code.
...
The workflow is a sequence of commands you drive from your AI coding assistant. Each step produces a version-controlled artifact that feeds the next.

image::sdd-workflow.png[Spec-driven development workflow: init, specify, clarify, plan, tasks, implement, build, inspect, deploy — with an iteration loop that refines the spec and regenerates.]
...
== Defining done with exit conditions

Spec-driven development expresses "done" as machine-checkable xref:sdk:exit-conditions.adoc[exit conditions], organized by xref:sdk:definition-of-done-types.adoc[Definition-of-Done type] and evaluated by xref:sdk:auditors.adoc[auditors]. Depending on the xref:sdk:conformance-modes.adoc[conformance mode], the definition of done is either a hard gate (Enforced) or an advisory checklist (À la carte). When a feature ships, a xref:sdk:shipping-and-receipts.adoc[conformance receipt] records what was verified and what defers downstream.
