...
The fastest way to start a Spec-Driven project is through the AI Marketplace plugin for your coding assistant. Install the **Akka plugin** from your assistant's marketplace, and SDD commands (`/akka.specify`, `/akka.clarify`, `/akka.plan`, etc.) become available immediately.

[TIP.manual]
.Without the AI Marketplace
====
If the AI Marketplace is not available in your environment, fall back to the Akka CLI:

. Install the Akka CLI (see xref:operations:cli/installation.adoc[]).
. Run `akka specify init` to scaffold an empty Akka project with all SDD templates.
...
Akka makes it easy to get started with a new _Spec-Driven_ project. Instead of using `akka code init`, use the `akka specify init` command. This gives you everything you need to kick off your Spec-Driven development cycle.

Refer to xref:operations:cli/installation.adoc[] for installation instructions.

=== Initialization and constitution adoption
Running `akka specify init` populates a project directory with an empty Akka project. In addition, it grabs all of the markdown templates necessary for the spec-driven process. Once you have run this command, change to the directory of your new project (if you are not already there) and launch your favorite code assistant.
