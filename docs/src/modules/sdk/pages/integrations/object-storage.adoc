...
<2> Create an `ImageUriMessageContent` referencing the stored object via an `object://` URI — no download happens yet.
...
Objects stored in buckets can be passed directly to AI agents as multimodal content. The factory methods `ImageMessageContent.create(bucket, key)` and `PdfMessageContent.create(bucket, key)` produce content references using an `object://` URI scheme. The SDK loads the data from the bucket automatically before sending the request to the model.
