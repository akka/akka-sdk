...
Object storage lets you store and retrieve binary objects — images, PDFs, documents, or any other binary data — in named buckets. The SDK provides a built-in `ObjectStoreProvider` that you can inject into any side-effecting component (endpoints, workflows, agents, consumers, timed actions).
...
== Getting an ObjectStore

Inject `ObjectStoreProvider` into your component and call `forBucket` to obtain an `ObjectStore` for a specific bucket:
...
----
<1> `ObjectStoreProvider` is injected by the SDK — no manual wiring needed.
<2> `forBucket` returns an `ObjectStore` scoped to the named bucket.

== Storing and retrieving objects

`ObjectStore` has a straightforward API designed for small to medium size binary payloads, plus streaming variants for large objects.
