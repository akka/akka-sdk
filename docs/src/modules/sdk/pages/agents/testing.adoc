...
Testing agents built with Generative AI involves two complementary approaches: evaluating the quality of the non-deterministic model behavior and writing deterministic unit tests for the agent's and surrounding components' logic. Evaluations are described in xref:sdk:agents/llm_eval.adoc[]; this page covers the deterministic testing.
