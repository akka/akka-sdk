...
Testing agents built with Generative AI involves two complementary approaches: evaluating the quality of the non-deterministic model behavior and writing deterministic unit tests for the agent's and surrounding components' logic. Evaluations are described in xref:testing:evaluation/index.adoc[] and xref:sdk:agents/llm_eval.adoc[]; this page covers the deterministic testing.
...
== Mocking judgment responses

An agent that uses `effects().judgment()` has a `JudgmentModelProvider` instead of a `ModelProvider`, see xref:sdk:agents/judgment.adoc[Structured judgment]. Replace it with link:{attachmentsdir}/testkit/akka/javasdk/testkit/TestJudgmentModelProvider.html[TestJudgmentModelProvider], which answers the questions without calling a SystemOne endpoint.

[source,java,indent=0]
.{sample-base-url}/doc-snippets/src/test/java/com/example/application/TriageAgentIntegrationTest.java[TriageAgentIntegrationTest.java]
----
include::example$doc-snippets/src/test/java/com/example/application/TriageAgentIntegrationTest.java[tag=judgment-test]
----
<1> Register the test provider for the agent with `withJudgmentModelProvider`.
<2> `whenState` matches the state the agent sends, as text. `fixedAnswers` answers every request.
<3> Reply with one answer per question key. The helper methods `choice`, `score`, `yes` and `no` build answers with full confidence. Construct `Judgment.ChoiceAnswer`, `Judgment.ScoreAnswer` or `Judgment.YesNoAnswer` directly for other probabilities.
<4> A request that matches no rule fails the call, which is how you test `onFailure`.
