...
When combining filters with `readLast()`, the filter is applied first to select matching messages, and then the limit is enforced on the filtered results.

==== Available filter types

The `MemoryFilter` interface provides several static factory methods:

* `MemoryFilter.includeFromAgentId(String id)` - Include only messages from the specified agent component ID
* `MemoryFilter.includeFromAgentId(Set<String> ids)` - Include only messages from multiple agent component IDs
* `MemoryFilter.excludeFromAgentId(String id)` - Exclude messages from the specified agent component ID
* `MemoryFilter.excludeFromAgentId(Set<String> ids)` - Exclude messages from multiple agent component IDs
* `MemoryFilter.includeFromAgentRole(String role)` - Include only messages from agents with the specified role
* `MemoryFilter.includeFromAgentRole(Set<String> roles)` - Include only messages from agents with multiple roles
* `MemoryFilter.excludeFromAgentRole(String role)` - Exclude messages from agents with the specified role
* `MemoryFilter.excludeFromAgentRole(Set<String> roles)` - Exclude messages from agents with multiple roles

Multiple filters can be applied simultaneously by passing them as varargs to methods like `filtered()`, `readOnly()`, or `readLast()`.
