...
You can chain multiple filters together using the fluent builder API. Filters of the same type are automatically merged:

[source,java,indent=0]
----
include::example$doc-snippets/src/main/java/com/example/application/MyAgentMore.java[tag=filter-multiple]
----
<1> The two `includeFromAgentId` calls are merged into a single filter that includes messages from both "agent-1" and "agent-2". Then the `excludeFromAgentRole` filter excludes any messages with the "internal" role from the combined result.
