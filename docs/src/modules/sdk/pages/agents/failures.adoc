...
A structured judgment call, see xref:sdk:agents/judgment.adoc[Structured judgment], raises the model-related exceptions and `GuardrailException`. It does not call tools and does not parse a response, so the tool execution and response processing exceptions do not occur.
