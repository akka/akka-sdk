...
Existing `TextGuardrail` implementations still run.
...
A `TextGuardrail`, including the deprecated built-in `SimilarityGuard`, requires the `use-for` property.
It accepts one or more of `model-request`, `model-response`, `mcp-tool-request`, and `mcp-tool-response`, or `*` for all four.
The `model-request` and `model-response` values are deprecated. A `model-request` guardrail becomes a `ModelCallGuardrail`, and a `model-response` guardrail becomes an `AgentResponseGuardrail`.
The `mcp-tool-request` and `mcp-tool-response` values are not deprecated. A `TextGuardrail` is the only guardrail type that the runtime evaluates for MCP tool calls.
...
The built-in link:{attachmentsdir}/api/akka/javasdk/agent/ModelCallSimilarityGuard.html[`ModelCallSimilarityGuard`] is a `ModelCallGuardrail`. Before each model call, the runtime searches a dataset of "bad examples" for matches. It checks the user message and the tool results. If the similarity reaches the threshold, the runtime denies the call.

This is how to configure the `ModelCallSimilarityGuard`:
...
The SDK ships this guardrail as the built-in `"default model-call jailbreak"`, with no agents selected by default. To enable it, set `agents` or `agent-roles`:

[source,conf,indent=0]
.src/main/resources/application.conf
----
akka.javasdk.agent.guardrails {
  "default model-call jailbreak" {
    agents = ["planner-agent", "weather-agent"]
  }
}
----
...
[NOTE]
====
The link:{attachmentsdir}/api/akka/javasdk/agent/SimilarityGuard.html[`SimilarityGuard`] class and the built-in `"default jailbreak"` guardrail are deprecated.
They still work with `use-for`. See <<_configuring_a_deprecated_textguardrail,Configuring a deprecated TextGuardrail>>.
For `use-for = ["model-request"]`, use `ModelCallSimilarityGuard` or `"default model-call jailbreak"` instead.
For `mcp-tool-request` and `mcp-tool-response`, `SimilarityGuard` has no replacement. Keep using `SimilarityGuard` for them.
To move off `"default jailbreak"`, enable `"default model-call jailbreak"` and remove `"default jailbreak"` from your agents. Do not change the `class` of `"default jailbreak"`. It keeps its `use-for` from the SDK defaults. A `ModelCallSimilarityGuard` must not define `use-for`.
====
