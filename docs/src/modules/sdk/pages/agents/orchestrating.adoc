...
Let's first look at how to define a workflow that orchestrates several agents in a predefined steps. This is similar to the xref:sdk:agents/calling.adoc[`ActivityAgentManager`] that was illustrated above, but it uses both the `WeatherAgent` and the `ActivityAgent`. First it retrieves the weather forecast and then it finds suitable activities.
...
[TIP]
====
The same outcome, letting the model decide which agents to invoke and in what order, can often be expressed more directly with an xref:sdk:autonomous-agents.adoc[Autonomous Agent] coordinator that declares a xref:sdk:autonomous-agents/capabilities.adoc#_delegation_capability[`Delegation`] capability. The runtime drives the iteration loop, exposes the worker agents as tools to the model, persists task state, and retries failed steps. Stay with the planner-workflow approach below when you need explicit workflow-step boundaries, per-step retry policies, or external compensation between agent calls. The xref:getting-started:planner-agent/dynamic-team.adoc[planner-agent tutorial] walks through the autonomous-agent variant end-to-end.
====
...
.SelectorAgent.java
----
include::example$doc-snippets/src/main/java/agent_guide/dynamic_planner/SelectorAgent.java[tag=class]
----
...
Note that in image:concepts:steps-2.svg[width=20] of the `Selector` we retrieve a subset of the agents with a certain
 role. This role is also defined in the `@AgentRole` annotation. Tag the workers with `@AgentRole("worker")` so the `SelectorAgent` can discover them through the registry.

The result from the `Selector` is a list of agent ids:
[source,java,indent=0]
.AgentSelection.java
----
include::example$doc-snippets/src/main/java/agent_guide/dynamic_planner/AgentSelection.java[tag=all]
----
...
.PlannerAgent.java
----
include::example$doc-snippets/src/main/java/agent_guide/dynamic_planner/PlannerAgent.java[tag=class]
----
...
----
include::example$doc-snippets/src/main/java/agent_guide/dynamic_planner/AgentTeamWorkflow.java[tag=plan]
----
...
When executing the plan and calling the agents we know the id of the agent to call, but not the agent class. It can be the `WeatherAgent` or `ActivityAgent`. Therefore, we can't use the ordinary `method` of the `ComponentClient`. Instead, we use the `dynamicCall` with the id of the agent. We don't have compile time safety for those dynamic calls, but we know that these agents take an `AgentRequest` parameter and return `String`. If we used it with the wrong types, it would be a runtime exception.

[source,java,indent=0]
----
include::example$doc-snippets/src/main/java/agent_guide/dynamic_planner/AgentTeamWorkflow.java[tag=dynamicCall]
----

For an autonomous-agent variant of the same multi-agent problem, see the xref:getting-started:planner-agent/dynamic-team.adoc[planner-agent tutorial] and the link:https://github.com/akka-samples/multi-agent[akka-samples/multi-agent, window="new"] repository.
