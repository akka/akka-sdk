...
If you need the media to survive across requests — so the model can refer back to it later, or so it is available outside the conversation — persist it yourself in the tool: inject an `ObjectStorageProvider`, store the bytes in a bucket, and return an `ImageUrlMessageContent` or `PdfUrlMessageContent` referencing the stored object (for example via `ImageUrlMessageContent.create(bucket, key)`). The `object://` reference *is* persisted in session memory, and the SDK resolves it back to the bytes on each request.
====
