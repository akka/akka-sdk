...
Each binding declares a `trigger` — currently only `interaction`, meaning the evaluator runs for each interaction of the bound agent:
...
Under `agent-roles`, use `*` to bind every agent that has a role. The most specific binding decides for an agent: its entry under `agents`, then the entry for its role, then `*`. In the example, `billing-agent` is not evaluated even if it has the `customer-facing` role, because its own entry is disabled.

Both an evaluator and an individual binding may set `enabled` (default `true`); set it to `false` to disable the whole evaluator or a single binding.
...
For an evaluation that runs in several steps and must survive restarts, extend link:{attachmentsdir}/api/akka/javasdk/evaluation/DurableEvaluator.html[`DurableEvaluator`] instead of `Evaluator`.
A durable evaluator keeps state across its steps, and each evaluation runs as its own durable instance that resumes from the last completed step.

// TODO: document DurableEvaluator: steps and transitions, state, settings, and testing.
...
To go the other way, from an interaction to what was concluded about it, fetch its evaluations:

[source,java,indent=0]
.{sample-base-url}/doc-snippets/src/main/java/com/example/evaluator/LedgerUsage.java[LedgerUsage.java]
----
include::example$doc-snippets/src/main/java/com/example/evaluator/LedgerUsage.java[tag=evaluations]
----
<1> The evaluations recorded about that interaction, newest first. An evaluation is recorded when it terminates, so one that is still running is not in the list, and an interaction without evaluations returns an empty list.

The id to pass comes from `AgentReply.interactionId()`, which a call made with `withDetailedReply()` returns, or from `EvaluationContext.subject().interactionId()` inside an evaluator.
