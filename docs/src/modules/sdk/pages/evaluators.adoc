...
<4> `Subject.Interaction.interactionId()` identifies the interaction; fetch its record from the ledger.
...
`Subject` is a sealed interface with one variant per kind of thing an evaluator can be bound to: `Interaction` (one agent interaction, on its own or as part of a flow, identified by its stable `interactionId()` together with the `agentComponentId()` that produced it and, when part of a flow, its `flowId()`), `Flow` (a whole flow), `Session` (a whole conversation), `EvaluatedEvaluation` (a previously recorded evaluation), and `Experiment` (a whole, terminated experiment). Today only `Interaction` is fired by the runtime; the other kinds are read directly on `EvaluationContext` or resolved through a subject client.
