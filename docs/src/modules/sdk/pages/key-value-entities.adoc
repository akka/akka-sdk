...
image:ROOT:key-value-entity.png[Key value entity,width=100,float=left] xref:reference:glossary.adoc#key_value_entity[Key Value Entities] are entities that persist the full state on every change. Only the latest state is stored, so there is no access to the history of changes, unlike the event sourced storage used by xref:sdk:event-sourced-entities.adoc[Event Sourced Entities].
...
NOTE: The above uses the Java `record` to reduce boilerplate code, but regular classes work too as long as they can be serialized to JSON (e.g. using Jackson annotations). See xref:serialization.adoc[Serialization].
