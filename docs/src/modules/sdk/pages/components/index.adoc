...
If the entity is referenced by its id, do not create a View. Call the entity with the xref:sdk:component-and-service-calls.adoc[ComponentClient] and return its state from a read command handler. That read is strongly consistent and needs no extra component.
