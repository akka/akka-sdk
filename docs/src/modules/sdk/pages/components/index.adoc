= Components
:page-summary: The Akka components, with a guide for choosing between them: when to use each one, when not to, and what to use instead.
:page-when-to-use: When you need an overview of Akka's building blocks, or when you are mapping a feature to components and need to decide between entities, views, workflows, consumers, agents, timers, and endpoints.
...
To decide which component fits a given need, and when not to use one, see <<choosing-a-component>> below.
...
Decide the component mapping before writing code. Each section gives the default choice, the signals that should change it, and the signs that you chose wrong.
