...
Decide the component mapping before writing code. Each section gives the default choice, the signals that should change it, and the signs that you chose wrong. Each decision helper has a stable anchor (`cs1-entity-type` through `cs11-third-party-calls`) so that plans, reviews, and checklists can reference it.
