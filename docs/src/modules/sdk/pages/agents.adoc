...
* **Workflows** orchestrate multi-step processes and can invoke Agents as one of those steps. This is the request-based alternative to Autonomous Agent coordination capabilities: pick a Workflow when the orchestration sequence is fixed in code, pick xref:sdk:autonomous-agents.adoc[Autonomous Agents] when the model should decide what runs next.
* **Autonomous Agents** are the model-driven alternative for multi-step work and multi-agent coordination, with built-in task lifecycle and coordination capabilities. A request-based Agent is a single request-response interaction; an Autonomous Agent runs a durable process loop until each task completes. See xref:sdk:autonomous-agents.adoc[].
...
In the case of agents, you don't supply a unique identifier for the instance of the agent. Instead, you supply an identifier for the _session_ to which the agent is bound. This lets you have multiple components with different component IDs all performing various agentic tasks within the same shared session.
