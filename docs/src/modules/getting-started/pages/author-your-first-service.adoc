...
Run the request a few times to see the agent's responses. They will likely be very similar.

== Add your own greeting

Modify the HTTP Endpoint to accept a request body using the code shown below:

[source,java]
.HelloWorldEndpoint.java
----
include::java:example$doc-snippets/src/main/java/com/example/api/HelloWorldEndpoint.java[tag=helloUser]
----
...
curl -i -XPOST -H "Content-Type: application/json" localhost:9000/hello -d \
  '{"text":"Ola Mundo!", "user":"john"}'
----

Now, try again with a different language:

[source, command line]
----
curl -i -XPOST -H "Content-Type: application/json" localhost:9000/hello -d \
  '{"text":"Ciao mondo", "user":"john"}'
----

Run the request a few more times. Do you see the difference in the responses
as the Agent avoids repeating languages already used?
...
Akka Agents automatically track context using *session memory*. In this case, the Agent is able to remember all the past
messages and languages that were used in this session.
The session is identified by a unique identifier, which in this case is the user being passed in each request.
