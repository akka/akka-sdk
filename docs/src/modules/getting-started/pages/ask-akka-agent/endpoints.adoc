...
By the end of this step the service hosts its own web application: the browser loads the page from the same service that answers its questions, with no separate web server involved. See xref:sdk:web-applications.adoc[Web applications] for what else a service can serve to a browser.
