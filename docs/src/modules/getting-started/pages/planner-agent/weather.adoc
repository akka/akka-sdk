...
xref:planner-agent/index.adoc[1: The activity agent] > xref:planner-agent/preferences.adoc[2: User preferences] > **Weather agent** > xref:planner-agent/team.adoc[4: Orchestrate the agents] > xref:planner-agent/list.adoc[5: List by user]
...
.WeatherAgent.java
----
include::java:example$doc-snippets/src/main/java/agent_guide/part3/WeatherAgent.java[tag=class]
----
...
If you don't want to use the real weather service, you can change the implementation to return a hardcoded weather, such as `"It's always sunny"`.
...
.ActivityAgent.java
----
include::java:example$doc-snippets/src/test/java/agent_guide/part3/WeatherAgentIntegrationTest.java[tag=class]
----
...
Introduce a workflow that orchestrates the calls between the agents. Continue with xref:planner-agent/team.adoc[Part 4: Orchestrate the agents]
