...
| xref:planner-agent/dynamic-team.adoc[Part 5: Dynamic orchestration] — An Autonomous Agent coordinator that delegates dynamically to the worker agents using the built-in Delegation capability. | Advanced
| xref:planner-agent/eval.adoc[Part 6: Evaluating task results] — A Consumer subscribed to task-completion events runs LLM-as-judge and toxicity evaluators. | Intermediate
|=======================
