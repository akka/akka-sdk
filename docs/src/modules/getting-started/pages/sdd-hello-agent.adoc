...
To specify the main feature of the application, we use `/akka.specify`. The following prompt shows this in action. If you're feeling adventurous, you can experiment with changing the prompt to see how it affects the final results.
...
To generate the tasks from this plan, simply run `/akka.tasks`
...
== Implement the agent
Now it's time to generate some code. Just run the `/akka.implement` command and allow it to proceed with any operations for which it requests permission.
...
Exercising the agent API is easy. At this point, just run `/akka.build`. This will recompile your project, run tests, and issue a sample command to the RESTful API endpoint.
...
If you want to see how the code-first version of this tutorial unfolds, xref:getting-started:author-your-first-service.adoc[click here].
