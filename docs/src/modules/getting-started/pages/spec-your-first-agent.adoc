...
If you're using Claude Code, then you can take advantage of the plugin marketplace to install the https://github.com/akka/ai-marketplace[Akka specification plugin, window="new"], which will also automatically install and verify the Akka CLI for you.
...
NOTE: Not using Claude Code? Akka supports 31 different AI assist utilities. Visit xref:sdk:spec-driven-development.adoc[Spec-Driven Development] for details on how to configure each.
...
.Generated artifacts
[options="header", cols="2,2", width="100%"]
|===
| File | Purpose
| `specs/001-greeter-agent/plan.md` | Implementation plan with component design
| `specs/001-greeter-agent/research.md` | Decisions on session memory, model config, session ID strategy, error handling
| `specs/001-greeter-agent/data-model.md` | Data model (API records only, no entities)
...
* `HelloWorldAgent` — single Akka Agent with the verbatim system prompt, using SDK session memory and config-driven model provider
* `HelloWorldEndpoint` — POST /greet accepting `{"user": "...", "text": "..."}`, using user as session ID, returning 500 on model config failure
* No domain objects, no Entities, no Views — pure Agent + Endpoint
* Tests — Agent unit test with TestModelProvider, Endpoint integration test with httpClient

*Constitution check:* All four principles pass: Akka SDK First, Design Principles, Test Coverage, Simplicity.
...
.Task summary
[options="header", cols="2,3", width="100%"]
|===
| Metric | Value
| Total tasks | 8
...
*MVP scope:* User Stories 1 & 2 (Phase 3) — after just 5 tasks (T001–T005), the service is fully functional with a tested greeting flow.
...
[options="header", cols="1,1,4", width="100%"]
|===
| Task | Status | File
...
NOTE: You need `OPENAI_API_KEY` set for live LLM responses. Without it, the request returns a 500 error as designed.
