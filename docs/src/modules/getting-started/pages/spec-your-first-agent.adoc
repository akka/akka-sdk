...
* xref:quick-install-cli.adoc[Akka CLI] (_installed automatically if you use the Akka Claude Code plugin_)
...
IMPORTANT: From now on, any time you see `/akka.{command}` in the documentation referring to using Claude Code _without_ the marketplace plugin, you should substitute the `/akka-specify:{command}` plugin-based alternative.
...
To specify the main feature of the application, we use `/akka.specify` (`/akka-specify:specify` in the Claude Code plugin). The following prompt shows this in action. If you're feeling adventurous, you can experiment with changing the prompt to see how it affects the final results. Copy and paste this `/akka.specify` (or `/akka-specify:specify`) command into your agent session:
...
Copy and paste this slash command into your Claude session (use `/akka-specify:plan` if you're using the marketplace plugin):
...
To generate the tasks from this plan, run `/akka.tasks` (or `/akka-specify:tasks` when using the plugin)
...
== Implement the agent
Now it's time to generate some code. Run the `/akka.implement` (or `/akka-specify:implement` with the plugin) command and allow it to proceed with any operations for which it requests permission.
...
Exercising the agent API is easy. At this point, run `/akka.build` (or `/akka-specify:build`). This will recompile your project, run tests, and issue a sample command to the RESTful API endpoint.
