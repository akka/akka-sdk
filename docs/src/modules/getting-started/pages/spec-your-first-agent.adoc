...
This guide builds a "hello world" greeting agent end to end with spec-driven development: specify the feature, plan it, generate tasks, implement, and run it locally. The agent greets each user in a new language every turn and tracks the languages it has already used in the session. You drive the whole flow from your AI coding assistant.
...
Run `/akka:build`. This recompiles the project, runs the tests, starts the service, and issues a sample request to the endpoint. To save time, stop any running instance of the service and free port `9000` first.
