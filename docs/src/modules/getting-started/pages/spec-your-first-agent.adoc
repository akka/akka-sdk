...
── Project ──────────────────────────────────────────────────────
 🔒 Pipeline & Scanning — SAST + secrets clean ........ org policy
 ? Code & Test Health — coverage threshold ........... decision needed
── Process ──────────────────────────────────────────────────────
 🔒 Process Integrity — APIs cited, none invented ..... org policy
----

See xref:sdk:enforced-mode.adoc#dod-types[Definition-of-Done types] for what each category covers.
...
Answer in your own words -- accept the suggestions or set your own. Akka's runtime comfortably supports a tight latency target:
...
The engine adds one exit condition and wires a _declarative_ auditor that delegates to the project's own toolchain -- here `mvn -q compile`, whose non-zero exit is the red verdict:
...
READY_TO_SHIP — completion policy met
  18 conditions · 13 verified · 3 asserted by Akka · 1 deferred → staging
