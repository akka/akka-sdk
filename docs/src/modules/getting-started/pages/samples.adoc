...
| xref:getting-started:ask-akka-agent.adoc[AI agent that performs a RAG workflow]. Illustrates how to create embeddings for vector databases, how to consume LLMs and maintain conversation history, use RAG to add knowledge to fixed LLMs, and expose it all as a streaming service. It uses MongoDB Atlas and OpenAI. | link:https://github.com/akka-samples/ask-akka-agent[Github Repository] |Intermediate
| AI agent that creates personalized travel itineraries. Illustrates reliable interaction with an LLM using a workflow. Entities are used for durable state of user preferences and generated trips. | link:https://github.com/akka-samples/travel-agent[Github Repository] |Beginner
| AI agent that leverages an LLM to process medical discharge summaries and assign tags, while also enabling human verification and comparative analysis. Interactions from a workflow with an agent using OpenAI LLM. | link:https://github.com/akka-samples/medical-tagging-agent[Github Repository] |Intermediate
...
| AI agent that uses an LLM to analyze data from fitness trackers, medical records and other sensors. Integration with Fitbit and MongoDB Atlas. | link:https://github.com/akka-samples/healthcare-agent[Github Repository] |Intermediate
| xref:getting-started:build-and-deploy-shopping-cart.adoc[Build a shopping cart] | link:https://github.com/akka-samples/shopping-cart-quickstart[Github Repository] |Beginner
