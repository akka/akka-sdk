...
ifdef::todo[TODO: a more realistic sample here, possibly aligned with the HTTP sample above]
ifdef::todo[TODO: cover streaming calls with an example?]

In this sample we will make an action that does a call to the xref:grpc-endpoints.adoc[gRPC endpoints example] service, deployed with the service name `doc-snippets`.
...
ifdef::todo[TODO: recomendations for how to use other ways to handle the protos]

In our delegating service implementation:

[source,java,indent=0]
.{sample-base-url}/doc-snippets/src/main/java/com/example/callanotherservice/DelegatingGrpcEndpointImpl.java[DelegatingGrpcEndpointImpl.java]
----
include::java:example$doc-snippets/src/main/java/com/example/callanotherservice/DelegatingGrpcEndpointImpl.java[tag=delegating-endpoint]
----
