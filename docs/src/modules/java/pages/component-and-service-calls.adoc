...
It is also possible to do calls but not wait on their completion, using `ComponentClient.invokeAsync` instead returns
`CompletionStage<T>` which makes it possible to trigger multiple calls at once, without waiting for the first one to complete,
so that they will be processed in parallel.
