...
TIP: Typically, since the service protobuf descriptors need to be shared between the provider service and the consuming service, if they are not part of the same codebase it is a common practice to put the proto files in a separate project that is then used on the two services.

In our delegating service implementation:

[source,java,indent=0]
.{sample-base-url}/event-sourced-customer-registry-subscriber/src/main/java/customer/api/DelegateCustomerGrpcEndpoint.java[DelegateCustomerGrpcEndpoint.java]
----
