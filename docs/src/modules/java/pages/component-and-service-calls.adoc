...
Requests and responses are always serialized to JSON while they are transmitted between the client and the component.
...
Calling services deployed on *different* Akka projects or any other external HTTP server is also done with the `HttpClientProvider`. Instead of a service name, the protocol and full server name are used when calling `httpClientFor`. For example, `https://example.com` or `http://example.com`.
...
The service is expected to accept HTTPS connections and to run on the standard HTTPS port (443). For calling a service on a nonstandard
