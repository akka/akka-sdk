...
Next we add the `createAssistant` method. This is almost entirely made up of `langchain4j` code and not specific to Akka. The purpose of this function is to use langchain's `AiServices` builder class to set up retrieval augmentation (MongoDB) and the chat model (Open AI).
...
== Next steps
Next we'll create streaming endpoints that provide clients with realtime access to LLM output.
