...
The implementation class of the guardrail is configured with the `class` property. The class must implement the link:_attachments/api/akka/javasdk/agent/Guardrail.html[`Guardrail` interface]. The class may optionally have a constructor with a link:_attachments/api/akka/javasdk/agent/GuardrailContext.html[`GuardrailContext`] parameter, which includes the name and the config section for the specific guardrail.
...
The guardrail can be enabled for certain inputs or outputs with the `use-for` property. Possible values are `model-request`, `model-response`, `mcp-tool-request`, `mcp-tool-response`, `*`.
