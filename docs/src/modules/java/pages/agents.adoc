...
Some models, such as OpenAI and Google Gemini, have specific support for structured model responses according to a given JSON schema. To automatically include a JSON schema that corresponds to the response type you can use the `responseAs` with a `boolean` parameter.
...
<3> Use the `responseAs` method  to specify the expected response type and with the boolean parameter `includeJsonSchema` set to `true`.
