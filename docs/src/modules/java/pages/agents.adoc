...
[source,java]
.{sample-base-url}/doc-snippets/src/main/java/com/example/application/MyAgent.java[ActivityAgent.java]
----
include::example$doc-snippets/src/main/java/com/example/application/MyAgent.java[tag=prompt]
----
...
As an alternative to hard-coded prompts, there is a built-in prompt template entity. The advantage of using the prompt template entity is that you can change the prompts in runtime without restarting or redeploying the service.
...
[source,java]
----
include::example$doc-snippets/src/main/java/com/example/application/MyAgent.java[tag=call]
----
<1> Use `forAgent`.
...
tracking. You can use a new random UUID for each call if the agent doesn't collaborate with other agents or have a multi-step interaction with the AI model.
