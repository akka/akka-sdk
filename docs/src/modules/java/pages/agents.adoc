...
3. **Tools defined by remot MCP servers** - Register remote MCP servers to let the agent use tools they provide.
...
The `WeatherService` has a method annotated with `@FunctionTool` and can be eventually used by multiple agents:
...
When a tool class like `WeatherService` is shared across multiple agents:
...
the agent. By default, all tools provided by each added remote MCP server are added, but it is possible to filter
...
[source,java]
.{sample-base-url}/doc-snippets/src/main/java/com/example/application/RemoteMcpWeatherAgent.java[RemoteMcpAgent
.java]
----
