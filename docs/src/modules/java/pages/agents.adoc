...
To create a more flexible and autonomous agentic system you want to analyze the problem and dynamically come up with a plan. The agentic system should identify the tasks to achieve the goal by itself. Decide what agents to use and in what order to execute them. Coordinate input and output between agents and adjust the plan along the way.

There are several approaches for the planning, such as using deterministic algorithms or using AI also for the planning. We will look at how we can use AI for analyzing a request, selecting agents and in what order to use them.
...
. Decide in what order to use the agents and give each agent precise instructions for its task.

The `Selector` agent decides what agents to use:
...
After selecting agents, we use a `Planner` agent to decide in what order to use the agents and what precise request that each agent should receive to perform its single task.
