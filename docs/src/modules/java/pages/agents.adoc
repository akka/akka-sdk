...
Testing Ai quality is a subject that goes way beyond the scope of Akka. Testing with generative AI is difficult no matter what you're using to implement it. Interactions with an LLM are not _deterministic_. In other words, you will never get the same answer twice for the same prompt.
...
[source,java]
.{sample-base-url}/multi-agent/src/main/java/demo/multiagent/application/agents/AgentTeamTest.java[AgentTeamTest.java]
----
