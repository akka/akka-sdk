...
<2> Implementation class of the guardrail, must implement `akka.javasdk.agent.Guardrail`. The class may optionally have a constructor with a `Config` parameter, which is the config section for the specific guardrail.
