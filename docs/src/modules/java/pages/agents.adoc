...
<2> Implementation class of the guardrail, must implement `akka.javasdk.agent.Guardrail`. The class may optionally have a constructor with a `akka.javasdk.agent.GuardrailContext` parameter, which includes the config section for the specific guardrail.
<3> Enable this guardrail for agents with these component ids.
<4> Enable this guardrail for agents with these roles.
<5> The type of validation, such as JAILBREAK, PROMPT_INJECTION, PII, TOXIC, HALLUCINATED, NSFW, FORMAT.
<6> Where to use the guardrail, possible values are `model-request`, `model-response`, `mcp-tool-request`, `mcp-tool-response`, `all`.
