...
A specific guardrail implements the link:_attachments/api/akka/javasdk/agent/Guardrail.html[`Guardrail` interface]. It takes the input or output text as a parameter and a result if it passed the validation or not, including an explanation of why the decision was made. These results are included in metrics and traces. A guardrail can abort the interaction with the model, or only report the problem and continue anyway.

Guardrails are enabled by configuration, to be able to enforce at an enterprise level that certain guardrails are always used.

[source,conf,indent=0]
----
akka.javasdk.agent.guardrails {
  "pii guard" {                                     // <1>
    class = "com.example.guardrails.PiiGuard"       // <2>
  ...
  "toxic guard" {
    class = "com.example.guardrails.ToxicGuard"
    agent-roles = ["worker"]
    category = TOXIC
...
The implementation class of the guardrail is configured with the `class` property. The class must implement the link:_attachments/api/akka/javasdk/agent/Guardrail.html[`Guardrail` interface]. The class may optionally have a constructor with an link:_attachments/api/akka/javasdk/agent/GuardrailContext.html[`GuardrailContext`] parameter, which includes the name and the config section for the specific guardrail.
...
The guardrail can be enabled for certain inputs or outputs with the `use-for` property. Possible values are `model-request`, `model-response`, `mcp-tool-request`, `mcp-tool-response`, `all`.
...
Here, it's using predefined examples of jailbreak prompts in `guardrail/jailbreak/`. Those have been incorporated from https://github.com/verazuo/jailbreak_llms/[https://github.com/verazuo/jailbreak_llms, window="new"], but you can define your own examples and place in `src/main/resources/` directory. All text files in the configured `bad-examples-resource-dir` are included in the similarity search.
