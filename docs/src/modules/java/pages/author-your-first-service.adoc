...
* Generate a simple project from a template that follows the recommended xref:concepts:architecture-model.adoc#architecture[onion architecture].
...
mvn archetype:generate ^
  ...
  -DarchetypeArtifactId=helloworld ^
  -DarchetypeVersion={akka-javasdk-version}
----
--
...
The local console requires that you install the Akka CLI first.
...
. Once the console is running, you can access it in your browser at:
+
http://localhost:3000
