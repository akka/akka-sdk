...
In the realm of distributed systems, Akka embraces an at-least-once delivery guarantee, for components like Consumer or Views (view updaters). Message redeliveries are not just possible, but inevitable.
...
- The receiver must be able to deduplicate based on the token, which, is most cases, has same limitations. See xref:request-deduplication[request deduplication].
