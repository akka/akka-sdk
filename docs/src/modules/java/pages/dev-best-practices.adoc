...
In the realm of distributed systems, Akka 3 embraces an at-least-once delivery guarantee, for components like Consumer or Views (view updaters). Message redeliveries are not just possible, but inevitable.
...
To ensure system integrity, consumers must be capable of handling duplicate messages gracefully. Effective deduplication is not just an optimization - it's a core architectural requirement, transforming the potential complexity of message redeliveries into a predictable and manageable system behavior.
...
.{sample-base-url}/event-sourced-customer-registry/src/main/java/customer/application/CustomerByEmailView.java[CustomerByEmailView.java]
----
include::example$event-sourced-customer-registry/src/main/java/customer/application/CustomerByEmailView.java[tag=idempotent-update]
----
<1> Processing each event is idempotent. Duplicated events will not change the state of the view.
...
----
<1> `ValueMultiplied` event contains not only delta information under `multiplier` field but also pre-calculated `currentValue` of the counter.
...
NOTE: A monotonically increased sequence number is available only when consuming updates from Akka 3 components like Event Sourced Entity, Key Value Entity, or another Akka service. The sequence number is *not globally unique*, but unique per entity instance.

[source,java,indent=0]
.{sample-base-url}/event-sourced-counter-brokers/src/main/java/counter/application/CounterByValueViewDeduplication.java[CounterByValueViewDeduplication.java]
----
include::example$event-sourced-counter-brokers/src/main/java/counter/application/CounterByValueViewDeduplication.java[tag=seq-tracking]
----
<1> A view model keeps track of the last processed sequence number.
<2> When sequence number is not available there is no deduplication.
...
Keep in mind that a View row corresponds to a single entity instance, that's why we can use the sequence number as a deduplication token.

The same technique can be used not only for the Akka Views, but for any external storage mechanism. As the matter of fact, Akka Views has a built-in support for sequence number tracking, so you don't have to implement it yourself. The example above just shows how it can be done manually.
...
Another idea is to use CloudEvents ID as a deduplication token. The CloudEvents ID is generated by the Akka runtime and is available in the event metadata.

[source,java,indent=0]
----
include::example$doc-snippets/src/main/java/com/example/application/MyConsumer.java[tag=cloud-event-id]
----
<1> Get the CloudEvent ID from the event metadata.

Unfortunately, according to the CloudEvents https://github.com/cloudevents/spec/blob/v1.0.2/cloudevents/spec.md#id[specification], the ID is not guaranteed to be the same for redelivery of the same event, hence we can't use it for deduplication.
...
This simple solution reveals a few important limitations of the deduplication that are common across many distributed technologies. It's very expensive (and often not possible) to have a total deduplication of all incoming requests/commands. There will always be some constraints like:
 ...
 - the size of the collection, e.g. keep only last 1000 command IDs, like in the sample above,
