...
[source,java]
.{sample-base-url}/transfer-workflow/src/main/java/com/example/transfer/application/TransferView.java[TransferView.java]
----
include::example$transfer-workflow/src/main/java/com/example/transfer/application/TransferView.java[tag=view-from-workflow]
----
...
NOTE: For the messages to be correctly consumed in the view, there must be a `ce-subject` metadata associated with each message. This is required because for each message consumed from the topic there will be a corresponding view row. That view row is selected based on such `ce-subject`.
