...
NOTE: This implementation does not interact with any other components and has an hardcoded response for simplification purposes. Interacting with other components is covered in the next section.
...
[source,protobuf]
.{sample-base-url}/event-sourced-customer-registry/src/main/proto/shoppingcart/api/shopping_cart_endpoint.proto[shopping_cart_endpoint.proto]
----
...
[source,java]
.{sample-base-url}/event-sourced-customer-registry/src/main/java/customer/api/CustomerGrpcEndpointImpl.java[ShoppingCartEndpointImpl.java]
----
