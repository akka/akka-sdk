...
Aside from the route annotation, this is just regular, synchronous Java code. With many other frameworks, to use some other component in the service, you would have to create a future and manually wait for it. With Akka, we can juse use `invoke()` and get synchronous results from components:
...
== When to make asynchronous calls
_When in doubt, write synchronous code_. Trust that Akka will do the right thing and that the runtime is making the necessary optimizations.
