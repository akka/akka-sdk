...
The `effects().error` is later transformed into a `UserException`. The default behavior is to return an HTTP 400 error with the error message as a response body.
...
For more fine-tuned control over the error handling it's possible to catch the `UserException` and transform it into a proper HTTP error.
...
----
<1> Catching the `UserException` and transforming it into 400 Bad Request error.

Remember that the xref:component-and-service-calls.adoc#_akka_components[called component] can be on a different node. Only the `UserException` and its subtypes are serialized and sent over the network. The xref:serialization.adoc[Jackson] serialization is configured to ignore fields like stack trace or cause from the Java `java.lang.Throwable` class. Other exceptions are not serializable and will be transformed into a generic HTTP 500 error.

Using the `effects().error(userException)` method or simply throwing a `UserException` will have the same effect. It's possible to have a more dedicated exceptions that will be used to signal different situations. For example, you can create a `CounterLimitExceededException` that extends `UserException` and use it in the command handler.
...
----
<1> Define a custom exception that extends `UserException`. Inner classes must be static to be serializable.
...
All unexpected exception (that doesn't extend `UserException`) thrown by the user code are transformed into an HTTP 500 error. When running the service locally in dev mode, a stack trace will be a part of the HTTP response. In production, this information is hidden, to not leak internal details about the service to a client. The client will receive a non-descriptive message with a correlation ID, like below.
