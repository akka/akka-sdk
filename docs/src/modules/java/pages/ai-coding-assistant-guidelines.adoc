...
* The `applyEvent` override should be a pure function transferring data from the event to the state and nothing else. It should will never fail, because validations should have been made before persisting the event.

never return null. It should always return the current state as a fall through default case or if something unexpected failed, it should throw an exception.
