...
* The `applyEvent` override should never return null. It should always return the current state as a fall through default case or if something unexpected failed, it should throw an exception.
