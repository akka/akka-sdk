...
_Patterns and conventions for consistency, maintainability, and legibility_
...
* Classes should use a suffix on the class name to indicate their respective roles within the application. Suffixes can include (but are not limited to):
** Entity
** View
** Workflow
** Consumer
...
* Command handlers are to be implemented in the entity and not in the state or domain object. Command handlers should invoke the domain object’s validators and methods that encapsulate the core business logic. The entity’s command handler is responsible for choosing the appropriate effect(s) to return. The domain object should not return effects or a list of events. This keeps the domain model clean of infrastructural concerns and makes it easy to unit test in isolation.
...
* State of an entity can be defined as a record inside the entity, but if it contains much business logic it should be in the `domain` package, also known as domain object.
* The `applyEvent` override should never return null. It should always return the current state as a fallthrough default case or if something unexpected failed, it should throw an exception.
* Entity events should:
** Be defined as records that implement a common sealed interface
...
* Prefer using parameterless methods to create each step of the workflow rather than defining them directly in the `definition` method. The methods should be named the same as the step and translate from the step name strings using Java camel case, e.g. a step named `"begin-batch"` will be produced from a method named `beginBatchStep()`.
...
** Omit root path if there is no common path for the endpoint methods.
