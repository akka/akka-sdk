...
In the link:_attachments/api/akka/javasdk/eventsourcedentity/ReplicationFilter.html[`ReplicationFilter`] you define the regions to be included or excluded in the replication. The region where the update is made, the so-called self region, is automatically included in the replication filter. The changes are addative, meaning that if you first `updateReplicationFilter` and include `gcp-us-east1` and then later make another ``updateReplicationFilter` and include  `aws-us-east-2` from the same entity, then both `gcp-us-east1` and `aws-us-east-2` are included.

When you add `@EnableReplicationFilter` the entity will still replicate to all regions until you have defined a filter with `updateReplicationFilter`. You can define the filter when persisting events, including the first event. For example, this is how to effectively disable replication to other regions for a specific entity by defining a filter that only includes the self region:
...
The skeleton of an Integration Test is included in the xref:getting-started:author-your-first-service.adoc[getting started sample]. Let's see what it could look like to test our `ShoppingCartEntity`:
