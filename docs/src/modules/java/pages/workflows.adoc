...
The Workflow can be used not only to orchestrate Akka components, but also to call external services. The step  invoke an `call` method can xref:component-and-service-calls.adoc#_external_http_services[HTTP endpoint], a xref:component-and-service-calls.adoc#_external_grpc_services[gRPC service], or any other service that can be called from the Java code.
