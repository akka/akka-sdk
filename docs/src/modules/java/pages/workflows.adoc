...
NOTE: Remember to cancel the timer once the Workflow is resumed. If you need a global workflow timeout, consider using durable timers as described in the xref:#_timeouts[Timeouts] section.
...
Workflows do not have a global timeout and can run indefinitely. Only step timeouts are supported.
