...
<2> Using the xref:component-and-service-calls.adoc#_component_client[ComponentClient], which is injected in the constructor, we instruct Akka to run a given call to withdraw funds from a wallet.
<3> The state of the workflow can be accessed with `currentState()` method.
<4> After successful withdrawal we return a `StepEffect` that will update the workflow state and move to the next step defined in `TransferWorkflow::depositStep` method. An input parameter for this step is a `Deposit` record.
<5> Another workflow step implementation to deposit funds to a given wallet.
<6> This time we return an effect that will stop workflow processing, by using the special `thenEnd` method.
...
By default, a workflow run has no time limit. It can run forever, which in most cases is not desirable behavior. A workflow step, on the other hand, has a default timeout of 5 seconds. Both settings can be overridden in the workflow `configuration` method.
