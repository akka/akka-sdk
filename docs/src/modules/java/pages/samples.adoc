...
| xref:java:ask-akka/index.adoc[AI agent that performs a RAG workflow] using Langchain4J. Chat memory by using entity and view. | link:https://github.com/akka-samples/ask-akka-agent[Github Repository] |Intermediate
| AI agent that creates personalized travel itineraries. Illustrates reliable interaction with a LLM using Langchain4J and a workflow. Entities are used for durable state of user preferences and generated trips. | link:https://github.com/akka-samples/ask-akka-agent[Github Repository] |Beginner
