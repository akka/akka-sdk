...
Accessing query parameter is also done through the `requestContext()`, inherited from link:_attachments/api/akka/javasdk/http/AbstractHttpEndpoint.html[AbstractHttpEndpoint].
...
----
<2> `queryParams().get("name")` returns an `Optional` which is empty if the query param is not present.
