...
[source,java]
.{sample-base-url}/doc-snippets/src/main/java/com/example/api/StaticResourcesEndpoint[StaticResourcesEndpoint.java]
----
...
NOTE: This is convenient for service documentation or small self-contained services with web UIs but is not intended
 for production, where coupling of the service lifecycle with the UI would mean that a new service version would need to be deployed for any changes in the UI.
