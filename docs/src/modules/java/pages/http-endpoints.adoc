...
be turned into a SSE endpoint method.
...
----
<1> `Source.tick` emits th element `"tick"` immediately (after `Duration.ZERO`) and then every 5 seconds
...
[source,java]
.{sample-base-url}/k-snippets/src/main/java/com/example/api/ExampleEndpoint.java[ExampleEndpoint.java]
----
