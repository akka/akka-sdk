...
1. Configure your AI assistant (Cursor, Qodo, etc.) to use this documentation
1. Include relevant sample code as additional context
1. Use our coding guidelines template for better code generation
1. Follow the prompt examples for common Akka development tasks
...
Alternatively, you could use `https://doc.akka.io/java/` as the base URL for the custom docs, and it would crawl the HTML pages. It's important to point at `https://doc.akka.io/java/`, and not `https://doc.akka.io/` since the latter also includes documentation about the Akka libraries that you don't want to use when developing with the Akka SDK.
...
1. Download the link:../java/_attachments/akka-docs-md.zip[akka-docs-md.zip].
1. Unzip the content and place in a folder `akka-context/docs` in your development project.
1. Add `akka-context/` to your `.gitignore` file, if you use git.
...
1. Pick one or a few samples from xref:java:samples.adoc[], which are relevant to what you are developing. If you are just getting started learning Akka you can pick the Shopping Cart sample.
1. Clone the sample Github repository. Pull latest if you have already cloned the repository before.
1. Copy the source code to a folder `akka-context/` in your development project, e.g. `akka-context/travel-agent/src`.
1. Add `akka-context/` to your `.gitignore` file, if you use git.
