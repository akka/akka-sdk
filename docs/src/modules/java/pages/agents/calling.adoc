...
The session id is used by the xref:java:agents/memory.adoc[session memory], but it is also important for observability tracking and AI evaluation.
...
We will look more at xref:java:agents/orchestrating.adoc[multi-agent systems], but let's start with a workflow for the single activities agent.
